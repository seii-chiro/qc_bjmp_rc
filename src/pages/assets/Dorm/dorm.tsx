...
const Dorm = () => {
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this dorm?",
  ...
  const handleExportPDF = () => {
    ...
    const tableData = (isSearching ? filteredData || [] : dataSource || []).map(
      (item, idx) => [idx + 1, item.cell_no, item.cell_name, item.floor]
    );
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleUpdate}>
          ...
          <Form.Item
            ...
            name="cell_no"
            rules={[
              { required: true, message: "Please enter the Dorm number" },
            ]}
          >
            <Input type="number" />
          </Form.Item>
          ...
          <Form.Item label="Dorm Description" name="cell_description">
            <Input />
          </Form.Item>
        </Form>
      </Modal>
