...
const jailfacility = () => {
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this jail facility?",
  ...
  const { mutate: editJail, isLoading: isUpdating } = useMutation({
    mutationFn: (updated: Jail) => updateJail(token ?? "", updated.id, updated),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["jail"] });
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: (a, b) =>
        a.jail_city_municipality.localeCompare(b.jail_city_municipality),
    },
    {
      title: "Barangay",
  ...
  const handleExportPDF = () => {
    ...
    const addHeader = () => {
      ...
      doc.setFontSize(16);
      doc.text("Jail Facility Report", 10, 15);
    ...
    for (let i = 0; i < tableData.length; i += maxRowsPerPage) {
      ...
      autoTable(doc, {
        head: [
          ["No.", "Jail", "Jail Type", "Jail Capacity", "Email", "Contact No."],
        ],
        body: pageData,
        startY: startY,
            ...
            >
              <Select
                ...
                options={jailProvinceData?.results
                  ?.filter(
                    (province) => province.region === selectjail?.jail_region_id
                  )
                  .map((province) => ({
                    value: province.id,
                    label: province?.desc,
            ...
            >
              <Select
                ...
                options={jailMunicipalityData?.results
                  ?.filter(
                    (municipality) =>
                      municipality.province === selectjail?.jail_province_id
                  )
                  .map((municipality) => ({
                ...
                options={jailBarangayData?.results
                  ?.filter(
                    (barangay) =>
                      ...
                      selectjail?.jail_city_municipality_id
                  )
                  .map((barangay) => ({
                    value: barangay.id,
                    label: barangay?.desc,
