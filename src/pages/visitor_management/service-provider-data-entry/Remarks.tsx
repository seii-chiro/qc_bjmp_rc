...
const Remarks = ({
  ...
  const [remarksTableInfo, setRemarksTableInfo] = useState<
    ServiceProviderRemarksForm[]
  >([]);
  const [editingRemark, setEditingRemark] = useState<{
    index: number;
    data: ServiceProviderRemarksForm;
  } | null>(null);
  ...
  const remarksDataSource = remarksTableInfo?.map((remarks, index) => {
    return {
      ...
      remarks:
        spRemarks?.results?.find((remark) => remark?.id === remarks?.remark)
          ?.remark ?? "N/A",
      actions: (
