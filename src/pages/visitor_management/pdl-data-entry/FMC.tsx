      ...
      onOk: () => {
        setPdlForm((prev) => ({
          ...
          person_relationship_data:
            prev.person_relationship_data?.filter((_, i) => i !== index) || [],
        }));
      },
    });
  };

  const contactDataSource = pdlForm?.person_relationship_data?.map(
    (item, index) => {
      return {
        key: index,
        relationship: relationships?.results?.find(
          (relationship) => relationship?.id === item?.relationship_id
        )?.relationship_name,
  ...
  return (
    <div className="flex flex-col gap-5 mt-10">
      ...
      <Modal
        ...
        title={
          editIndex !== null
            ? "Edit Family, Relatives & Contacts"
            : "Add Family, Relatives & Contacts"
        }
        open={isModalOpen}
