...
const CaseDetailsForm = ({
  ...
  useEffect(() => {
    if (casesForm?.court_branch_id) {
      setCasesForm((prev) => ({
        ...
        judge:
          courtBranches?.find(
            (branch) => branch?.id === casesForm?.court_branch_id
          )?.judge ?? "Vacant",
        court_name:
          courtBranches?.find(
            (branch) => branch?.id === casesForm?.court_branch_id
  ...
  useEffect(() => {
    if ((casesForm?.offense_id && offenses && crimeCategories, laws)) {
      const matchedOffense = offenses?.find(
        (offense) => offense.id === casesForm.offense_id
      );
      const matchedCategory = crimeCategories?.find(
        (category) =>
          category.crime_category_name === matchedOffense?.crime_category
      );
      const matchedLaw = laws?.find((law) => law.name === matchedOffense?.law);
  ...
  const handleSubmit = () => {
    setPdlForm((prev) => {
      ...
      const sanitizedCase = removeNullFields(casesForm);

      if (editIndex !== null) {
        newCaseData[editIndex] = sanitizedCase as CasesDetailsForm; // update existing case
      } else {
        newCaseData.push(sanitizedCase as CasesDetailsForm); // add new case
      }
    ...
    message.success(
      editIndex
        ? "Successfully Updated Case Details"
        : "Successfully Added Case Details"
    );
      ...
      <form className="w-full flex flex-col justify-center items-center gap-1">
        ...
        <div className="flex justify-between gap-2 w-full items-center">
          ...
          <label className="flex flex-col flex-1">
            ...
            <Input value={casesForm?.court_name} readOnly className="h-12" />
          </label>
          <label className="flex flex-col flex-1">
            <span className="font-semibold">Judge</span>
            <Input value={casesForm?.judge} readOnly className="h-12" />
          </label>
          <label className="flex flex-col flex-1">
            ...
            <Input
              ...
              onChange={(e) =>
                setCasesForm((prev) => ({
                  ...prev,
                  bail_recommended: e.target.value,
                }))
              }
            />
          </label>
          <label className="flex flex-col flex-1">
            ...
            <DatePicker
              className="h-12"
              value={
                casesForm?.date_crime_committed
                  ? dayjs(casesForm.date_crime_committed)
                  : null
              }
              onChange={(date) => {
                setCasesForm((prev) => ({
        ...
        <div className="flex justify-between gap-2 w-full items-center">
          ...
          <label className="flex flex-col flex-1">
            ...
            <DatePicker
              className="h-12"
              value={
                casesForm?.assignment_date
                  ? dayjs(casesForm.assignment_date)
                  : null
              }
              onChange={(date) => {
                setCasesForm((prev) => ({
          ...
          <label className="flex flex-col flex-[3]">
            ...
            <Input
              ...
              className="h-12"
              onChange={(e) =>
                setCasesForm((prev) => ({ ...prev, remarks: e.target.value }))
              }
            />
          </label>
        </div>
