...
const IssueType = () => {
  ...
  const [selectIssueType, setSelctedIssueType] = useState<IssueTypes | null>(
    null
  );
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this issue type?",
  ...
  const dataSource =
    data?.results?.map((issue_type, index) => ({
      key: index + 1,
      id: issue_type?.id ?? "",
      ...
      categorization_rule:
        issue_type?.issue_category?.categorization_rule ?? "",
      issue_category_description: issue_type?.issue_category?.description ?? "",
      risk: issue_type?.risk?.name,
      recommended_action: issue_type?.risk?.recommended_action.map(
        (recommended_action) => recommended_action.name
      ),
      organization:
        issue_type?.organization ?? "Bureau of Jail Management and Penology",
  ...
  const columns: ColumnsType<IssueTypes> = [
    ...
    },
    {
      ...
      sorter: (a, b) =>
        a.issue_category_description.localeCompare(
          b.issue_category_description
        ),
    },
    {
      ...
      sorter: (a, b) =>
        a.categorization_rule.localeCompare(b.categorization_rule),
    },
    // {
    //     title: 'Risk',
    //     dataIndex: 'risk',
  ...
  const handleExportPDF = () => {
    ...
    const tableData = (isSearching ? filteredData || [] : dataSource || []).map(
      (item, idx) => [idx + 1, item.name, item.issue_category]
    );
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleUpdate}>
          <Form.Item
            ...
            rules={[
              { required: true, message: "Please input the issue type name" },
            ]}
          >
            <Input />
          </Form.Item>
          <Form.Item name="issue_category" label="Issue Category">
            <Select
          ...
          <Form.Item name="categorization_rule" label="Categorization Rule">
            <Input.TextArea rows={3} />
          </Form.Item>
          <Form.Item name="recommended_action" label="Recommended Action">
            <Input.TextArea rows={3} />
          </Form.Item>
