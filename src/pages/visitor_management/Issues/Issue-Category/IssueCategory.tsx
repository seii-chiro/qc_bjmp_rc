...
const IssueCategory = () => {
  ...
  const [selectIssueCategory, setSelctedIssueCategory] =
  ...
  const fetchIssueCategory = async (search: string) => {
    const res = await fetch(
      `${BASE_URL}/api/issues_v2/issue-categories/?search=${search}`,
      {
        headers: {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["category", "category-table", page, limit],
    queryFn: async (): Promise<PaginatedResponse<IssueCategory>> => {
      ...
      const res = await fetch(
        `${BASE_URL}/api/issues_v2/issue-categories/?${params.toString()}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Token ${token}`,
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this issue category?",
  ...
  const dataSource =
    data?.results?.map((issue_category, index) => ({
      key: index + 1,
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      for (let i = 0; i < tableData.length; i += maxRowsPerPage) {
        ...
        autoTable(doc, {
          head: [
            ["No.", "Issue Category", "Categorization Rule", "Description"],
          ],
          body: pageData,
          startY: startY,
    ...
    <div>
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleUpdate}>
          <Form.Item name="name" label="Issue Type Name">
            <Input />
          </Form.Item>
