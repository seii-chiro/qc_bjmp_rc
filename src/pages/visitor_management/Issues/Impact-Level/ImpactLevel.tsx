...
const ImpactLevel = () => {
  ...
  const [selectImpactLevel, setSelctedImpactLevel] =
    useState<ImpactLevelProps | null>(null);
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this impact level?",
  ...
  const dataSource =
    data?.results?.map((impact_level) => ({
      ...
      organization:
        impact_level?.organization ?? "Bureau of Jail Management and Penology",
  ...
  const handleExportPDF = () => {
    ...
    const tableData = (isSearching ? filteredData || [] : dataSource || []).map(
      (item, idx) => [idx + 1, item.impact_level, item.description]
    );
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleUpdate}>
          <Form.Item
            ...
            rules={[
              { required: true, message: "Please input the Impact Level name" },
            ]}
          >
            <Input />
          </Form.Item>
