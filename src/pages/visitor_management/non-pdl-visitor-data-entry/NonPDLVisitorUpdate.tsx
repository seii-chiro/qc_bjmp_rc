...
const patchNonPDLVisitor = async (
  visitor: Partial<NonPdlVisitorForm>,
  token: string,
  id: string
) => {
  const res = await fetch(
    `${BASE_URL}/api/non-pdl-visitor/non-pdl-visitors/${id}/`,
    {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
...
const NonPdlVisitorUpdate = () => {
  ...
  const {
    data: nonPDLVisitorData,
    isLoading: nonPDLVisitorDataLoading,
    error,
  } = useQuery({
    queryKey: ["Non PDL Visitor", nonPDLVisitorToEdit],
    queryFn: async () => {
      const response = await fetch(
        `${BASE_URL}/api/non-pdl-visitor/non-pdl-visitors/${nonPDLVisitorToEdit}/`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const patchPersonMutation = useMutation({
    mutationKey: [
      "patch-person-non-pdl-visitor",
      nonPDLVisitorData?.person?.id,
    ],
    mutationFn: () =>
      patchPerson(personForm, token ?? "", nonPDLVisitorData?.person?.id),
    onSuccess: async () => {
  ...
  useEffect(() => {
    setNonPdlVisitorForm((prev) => ({
      ...
      }`, //idk why but its what the api want?? or maybe i misread it 🦊
    }));
  }, [
  ...
  useEffect(() => {
    setPersonForm((prev) => ({
      ...
      nationality_id:
        nationalities?.find(
          (nationality) =>
            nationality?.nationality === nonPDLVisitorData?.person?.nationality
        )?.id ?? null,
      ...
      civil_status_id:
        civilStatuses?.find(
          (civilStatus) =>
            civilStatus?.status === nonPDLVisitorData?.person?.civil_status
        )?.id ?? null,
      address_data:
        nonPDLVisitorData?.person?.addresses?.map(
          (existingAddress: {
            region: string;
            province: string;
            municipality: string;
      ...
      media_identifier_data:
        nonPDLVisitorData?.person?.media_identifiers?.map(
          (prev: { idtype: number }) => ({
            ...prev,
    ...
    setNonPdlVisitorForm((prev) => ({
      ...
      non_pdl_visitor_type_id:
        visitorTypes?.find(
          (type) =>
            type?.non_pdl_visitor_type ===
            nonPDLVisitorData?.non_pdl_visitor_type
        )?.id ?? null,
      visitor_rel_personnel_id:
        relationships?.find(
          (rel) =>
            rel?.relationship_personnel ===
            nonPDLVisitorData?.visitor_rel_personnel
        )?.id ?? null,
      personnel_id: nonPDLVisitorData?.personnel?.id,
      visitor_status_id:
        visitorAppStatus?.find(
          (stat) => stat?.status === nonPDLVisitorData?.visitor_status
        )?.id ?? null,
      id_number: nonPDLVisitorData?.id_number,
      verified_by_id: nonPDLVisitorData ?? currentUser?.id ?? null,
      non_pdl_visitor_reason_id:
        reasons?.find(
          (res) =>
            res?.reason_visit === nonPDLVisitorData?.non_pdl_visitor_reason
        )?.id ?? null,
      remarks_data: nonPDLVisitorData?.remarks ?? [],
      approved_at: nonPDLVisitorData?.approved_at,
      verified_at: nonPDLVisitorData?.verified_at,
      approved_by_id:
        users?.find((user) => user?.id === +nonPDLVisitorData?.approved_by)
          ?.id ?? null,
    }));
  }, [
    nonPDLVisitorData,
