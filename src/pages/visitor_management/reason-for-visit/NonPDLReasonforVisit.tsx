...
const NonPDLReasonforVisit = () => {
  ...
  const [reasonforVisit, setreasonforVisit] =
    useState<ReasonforVisitRecord | null>(null);
  ...
  const fetchReasonforVisit = async (search: string) => {
    const res = await fetch(
      `${BASE_URL}/api/non-pdl-visitor/non-pdl-reason-visits/?search=${search}`,
      {
        headers: {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["reason", "reason-table", page, limit],
    queryFn: async (): Promise<PaginatedResponse<ReasonforVisitRecord>> => {
      ...
      const res = await fetch(
        `${BASE_URL}/api/non-pdl-visitor/non-pdl-reason-visits/?${params.toString()}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Token ${token}`,
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this reason for visit?",
  ...
  return (
    <div>
      ...
      >
        <AddReasonforVisit onClose={handleCancel} />
      </Modal>
      <Modal
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleUpdate}>
          <div className="flex flex-wrap">
            <div className="w-full md:w-1/2 p-2">
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "Please input the Reason for Visit",
                  },
                ]}
              >
                <Input className="h-12 border w-full border-gray-300 rounded-lg px-2" />
              </Form.Item>
              <Form.Item name="risks" label="Risks">
                <Input className="h-12 border w-full border-gray-300 rounded-lg px-2" />
              </Form.Item>
              <Form.Item
                name="impacts"
            ...
            <div className="w-full md:w-1/2 p-2">
              <Form.Item label="Impact Level" name="impact_level">
                <Select
              ...
              <Form.Item label="Threat Level" name="threat_level">
                <Select
                  className="h-[3rem] w-full"
                  showSearch
