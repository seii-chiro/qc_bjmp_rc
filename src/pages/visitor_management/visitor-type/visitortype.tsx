...
const VisitorType = () => {
  ...
  const [selectedVisitorType, setSelectedVisitorType] =
    useState<VisitorTypeRecord | null>(null);
  ...
  const dataSource =
    data?.results?.map((visitor_type, index) => ({
      key: index + 1,
      id: visitor_type.id,
      visitor_type: visitor_type?.visitor_type ?? "N/A",
      description: visitor_type?.description ?? "N/A",
      organization:
        visitor_type?.organization ?? "Bureau of Jail Management and Penology",
  ...
  const handleExportPDF = () => {
    ...
    const tableData = (isSearching ? filteredData || [] : dataSource || []).map(
      (item, idx) => [idx + 1, item.visitor_type, item.description]
    );
