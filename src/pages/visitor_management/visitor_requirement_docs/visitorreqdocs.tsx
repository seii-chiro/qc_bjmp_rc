...
const VisitorReqDocs = () => {
  ...
  const [visitorReqDocs, setVisitorReqDocs] = useState<VisitorReqDocs | null>(
    null
  );
  ...
  const dataSource =
    data?.results?.map((visitor_req_docs, index) => ({
      key: index + 1,
      id: visitor_req_docs?.id,
      document_name: visitor_req_docs?.document_name ?? "",
      description: visitor_req_docs?.description ?? "",
      organization:
        visitor_req_docs?.organization ??
  ...
  const handleExportPDF = () => {
    ...
    const addHeader = () => {
      ...
      doc.setFontSize(16);
      doc.text("Visitor Requirements Documents Report", 10, 15);
    ...
    const tableData = (isSearching ? filteredData || [] : dataSource || []).map(
      (item, idx) => [idx + 1, item.document_name, item.description]
    );
    ...
    for (let i = 0; i < tableData.length; i += maxRowsPerPage) {
      ...
      autoTable(doc, {
        head: [["No.", "Visitor Requirements Documents", "Description"]],
        body: pageData,
        startY: startY,
  ...
  return (
    <div className="h-screen">
      ...
      <h1 className="text-3xl font-bold text-[#1E365D]">
        Visitor Requirements Documents
      </h1>
      ...
      <Modal
        title="Visitor Requirement Documents Report"
        open={isPdfModalOpen}
