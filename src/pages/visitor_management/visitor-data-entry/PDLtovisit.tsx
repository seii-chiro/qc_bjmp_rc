...
const PDLtovisit = ({
  ...
  const [pdlToVisitTableInfo, setPdlToVisitTableInfo] =
    useState<PdlToVisitTable>([]);

  const [requirementIndexToEdit, setRequirementIndexToEdit] = useState<
    number | null
  >(null);
  ...
  const { data: pdlsPaginated, isLoading: pdlsLoading } = useQuery({
  ...
  const handleDeletePdl = (index: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this PDL?",
  ...
  const handleDeleteRequirement = (index: number) => {
    Modal.confirm({
      ...
      cancelText: "Cancel",
      onOk() {
        deleteMediaRequirementByIndex(index);
      },
    });
  };
  ...
  const handleDeleteIdentifier = (index: number) => {
    Modal.confirm({
      ...
      cancelText: "Cancel",
      onOk() {
        deleteMediaIdentifierByIndex(index);
      },
    });
  };
  ...
  const pdlToVisitDataSource = pdlToVisitTableInfo?.map((pdl, index) => {
    ...
    return {
      ...
      relationship:
        visitorToPdlRelationship?.find(
          (relation: { id: number | null }) =>
            relation?.id === pdl?.relationship
  ...
  const requirementDataSources = personForm?.media_requirement_data?.map(
    (requirement, index) => {
      return {
        key: index,
        requirement: requirement?.name,
        description: requirement?.media_data?.media_description,
        image: requirement?.media_data?.media_base64 ? (
          <FullScreen
            handle={requirementFullscreenHandle}
  ...
  const IdentifierDataSource = personForm?.media_identifier_data?.map(
    (identififier, index) => {
      return {
        key: index,
        requirement: idTypes?.find(
          (id: { id: number | null }) => id?.id === identififier?.id_type_id
        )?.id_type,
        description: identififier?.media_data?.media_description,
        image: identififier?.media_data?.media_base64 ? (
          <FullScreen
            handle={idFullscreenHandle}
            className="flex items-center justify-center"
          >
            <Image
  ...
  return (
    <div>
      ...
      >
        <RequirementsForm
          editRequirement={
            personForm?.media_requirement_data?.[
              requirementIndexToEdit ?? -1
            ] ?? null
          }
      ...
      >
        <IdForm
          editRequirement={
            personForm?.media_identifier_data?.[idIndexToEdit ?? -1] ?? null
          }
