...
import {
  BiometricRecordFace,
  CustomFingerResponse,
  FaceVerificationResponse,
} from "@/lib/scanner-definitions";
import {
  captureFace,
  getIrisScannerInfo,
  uninitIrisScanner,
...
const VisitorProfile = ({
  ...
  const { fingerScannerTimeout, irisScannerTimeout, nfiqQuality, allowForce } =
  ...
  //Face Related States
  const [faceVerificationResponse, setFaceVerificationResponse] =
    useState<FaceVerificationResponse | null>(null);

  //Iris Related States
  const [irisScannerReady, setIrisScannerReady] = useState(false);
  const [leftIrisVerificationResponse, setLeftIrisVerificationResponse] =
    useState<any | null>(null);
  const [rightIrisVerificationResponse, setRightIrisVerificationResponse] =
    useState<any | null>(null);

  //Fingerprint Related States
  const [fingerScanner, setFingerScannerReady] = useState(false);
  const [LeftFingerResponse, setLeftFingerResponse] =
    useState<CustomFingerResponse | null>(null);
  const [RightFingerResponse, setRightFingerResponse] =
    useState<CustomFingerResponse | null>(null);
  const [ThumbFingerResponse, setThumbFingerResponse] =
    useState<CustomFingerResponse | null>(null);

  const addOrUpdateLeftFinger = (newFingerData: any, index: number) => {
    setLeftFingerResponse((prev) => {
      const updatedFingers = prev?.CapturedFingers
        ? [...prev.CapturedFingers]
        : [];
      ...
      try {
        ...
        if (!isCancelled) {
          message.open({
            type: "success",
            content:
              "Issue successfully submitted! Go to Issues module to view complete details.",
            duration: 10,
          });
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const closeUpImage = mediaArray.find(
      (media: { media_description: string }) =>
        media?.media_description === "Close-Up Front Picture"
    )?.media_binary;

    const signatureImage = mediaArray.find(
      (media: { media_description: string }) =>
        media?.media_description === "Signature Picture"
    )?.media_binary;

    const fullBodyImage = mediaArray.find(
      (media: { media_description: string }) =>
        media?.media_description === "Full-Body Front Picture"
    )?.media_binary;

    const leftsideImage = mediaArray.find(
      (media: { media_description: string }) =>
        media?.media_description === "Left-Side View Picture"
    )?.media_binary;

    const rightsideImage = mediaArray.find(
      (media: { media_description: string }) =>
        media?.media_description === "Right-Side View Picture"
    )?.media_binary;
  ...
  useEffect(() => {
    ...
    const rightIris = biometricArray.find(
      (biometric: { position: string }) => biometric?.position === "iris_right"
    )?.data;
    const leftIris = biometricArray.find(
      (biometric: { position: string }) => biometric?.position === "iris_left"
    )?.data;

    const face = biometricArray.find(
      (biometric: { position: string }) => biometric?.position === "face"
    )?.data;

    const rightLittle = biometricArray.find(
      (biometric: { position: string }) =>
        biometric?.position === "finger_right_little"
    )?.data;
    const rightRing = biometricArray.find(
      (biometric: { position: string }) =>
        biometric?.position === "finger_right_ring"
    )?.data;
    const rightMiddle = biometricArray.find(
      (biometric: { position: string }) =>
        biometric?.position === "finger_right_middle"
    )?.data;
    const rightIndex = biometricArray.find(
      (biometric: { position: string }) =>
        biometric?.position === "finger_right_index"
    )?.data;
    const rightThumb = biometricArray.find(
      (biometric: { position: string }) =>
        biometric?.position === "finger_right_thumb"
    )?.data;

    const leftLittle = biometricArray.find(
      (biometric: { position: string }) =>
        biometric?.position === "finger_left_little"
    )?.data;
    const leftRing = biometricArray.find(
      (biometric: { position: string }) =>
        biometric?.position === "finger_left_ring"
    )?.data;
    const leftMiddle = biometricArray.find(
      (biometric: { position: string }) =>
        biometric?.position === "finger_left_middle"
    )?.data;
    const leftIndex = biometricArray.find(
      (biometric: { position: string }) =>
        biometric?.position === "finger_left_index"
    )?.data;
    const leftThumb = biometricArray.find(
      (biometric: { position: string }) =>
        biometric?.position === "finger_left_thumb"
    )?.data;
  ...
  const [fingerprintVerificationResult1, setFingerprintVerificationResult1] =
    useState<any | null>(null);
  const [fingerprintVerificationResult2, setFingerprintVerificationResult2] =
    useState<any | null>(null);
  const [fingerprintVerificationResult3, setFingerprintVerificationResult3] =
    useState<any | null>(null);
  const [fingerprintVerificationResult4, setFingerprintVerificationResult4] =
    useState<any | null>(null);
  const [fingerprintVerificationResult5, setFingerprintVerificationResult5] =
    useState<any | null>(null);
  const [fingerprintVerificationResult6, setFingerprintVerificationResult6] =
    useState<any | null>(null);
  const [fingerprintVerificationResult7, setFingerprintVerificationResult7] =
    useState<any | null>(null);
  const [fingerprintVerificationResult8, setFingerprintVerificationResult8] =
    useState<any | null>(null);
  const [fingerprintVerificationResult9, setFingerprintVerificationResult9] =
    useState<any | null>(null);
  const [fingerprintVerificationResult10, setFingerprintVerificationResult10] =
    useState<any | null>(null);
    ...
    onSuccess: async (data) => {
      ...
      if (data?.images?.icao) {
        if (data?.images?.gender !== inputGender) {
          message.warning(
            "The provided gender does not match the gender identified through face recognition."
          );
        }
        verifyFaceMutation.mutate({
          template: data?.images?.icao,
          type: "face",
        });
        verifyFaceInWatchlistMutation.mutate({
          template: data?.images?.icao,
          type: "face",
        });
      } else {
  ...
  const irisScannerInitMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data?.ErrorDescription === "Device not connected.") {
        message.error("Iris scanner not connected.")
      }
    },
    onError: (error) => {
      console.error(error.message);
  ...
  const verifyLeftIrisMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      message.info(
        data?.message === "Match found." ? "Match Found" : "No Matches Found"
      );
    },
    onError: () => {
  ...
  const handleThumbFingers = (img0: string | null, img1: string | null) => {
    ...
    if (img0) {
      verifyFingerprintMutation1.mutate({
        template: img0,
        type: "fingerprint",
      });
    }
    ...
    if (img1) {
      verifyFingerprintMutation6.mutate({
        template: img1,
        type: "fingerprint",
      });
    }
  ...
  const handleVerifyRightFingers = (
    ...
    if (img0)
      verifyFingerprintMutation7.mutate({
        template: img0,
        type: "fingerprint",
      });
    if (img1)
      verifyFingerprintMutation8.mutate({
        template: img1,
        type: "fingerprint",
      });
    if (img2)
      verifyFingerprintMutation9.mutate({
        template: img2,
        type: "fingerprint",
      });
    if (img3)
      verifyFingerprintMutation10.mutate({
        template: img3,
        type: "fingerprint",
      });
  };
  ...
  const handleVerifyLeftFingers = (
    ...
    if (img0)
      verifyFingerprintMutation2.mutate({
        template: img0,
        type: "fingerprint",
      });
    if (img1)
      verifyFingerprintMutation3.mutate({
        template: img1,
        type: "fingerprint",
      });
    if (img2)
      verifyFingerprintMutation4.mutate({
        template: img2,
        type: "fingerprint",
      });
    if (img3)
      verifyFingerprintMutation5.mutate({
        template: img3,
        type: "fingerprint",
      });
  };
    ...
    setPersonForm((prev) => {
      const updatedMediaData = [
        ...(prev.media_data || []).filter(
          (media) =>
            ![
              "Close-Up Front Picture",
              "Full-Body Front Picture",
              "Left-Side View Picture",
              "Right-Side View Picture",
              "Signature Picture",
            ].includes(media.media_description || "")
        ),
      ];
    ...
    <div className="w-full mt-5">
      ...
      >
        <div className="w-full flex flex-col mt-8 rounded">
          <WebcamFullBody
            key={webcamKey}
            onCapture={setImageFullBody}
            setCameraModalOpen={setCameraFullBodyModalOpen}
          />
        </div>
      </Modal>
      <Modal
      ...
      >
        <div className="w-full flex flex-col mt-8 rounded">
          <CustomWebCam
            key={webcamKey}
            onCapture={setImageLeftSide}
            setCameraModalOpen={setCameraLeftModalOpen}
          />
        </div>
      </Modal>
      <Modal
      ...
      >
        <div className="w-full flex flex-col mt-8 rounded">
          <CustomWebCam
            key={webcamKey}
            onCapture={setImageRightSide}
            setCameraModalOpen={setCameraRightModalOpen}
          />
        </div>
      </Modal>
                  ...
                  >
                    ...
                    <div className="border border-gray-200 bg-gray-100 w-full flex-1 rounded-md overflow-hidden relative">
                      {leftIrisVerificationResponse &&
                        ...
                        (leftIrisVerificationResponse?.message ===
                        "Match found." ? (
                    ...
                    <div className="border border-gray-200 bg-gray-100 w-full flex-1 rounded-md overflow-hidden relative">
                      {rightIrisVerificationResponse &&
                        ...
                        (rightIrisVerificationResponse?.message ===
                        "Match found." ? (
                          <p className="h-4 w-4 text-sm bg-red-500 absolute right-0"></p>
                        ) : (
                          <p className="h-4 w-4 text-sm bg-green-500 absolute right-0"></p>
                      ...
                      <div className="absolute bottom-0 left-0 w-full px-2 py-1 flex justify-between">
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          >
                            {irisScannerInitMutation.isPending ? (
                              <Spin />
                            ) : (
                              <IoIosCamera
                                className={clsx(
                                  "text-2xl hover:scale-125 transition-all ease-in-out hover:text-blue-500",
                                  irisScannerReady
                                    ? ""
              ...
              <div className="w-[33.2rem] h-[26.5rem]">
                <div className="relative border border-gray-200 gap-6 flex flex-col items-center w-full rounded-md h-[26.5rem] object-cover">
                  <div className="w-full h-[90%]">
                    ...
                    >
                      {faceVerificationResponse &&
                        (faceVerificationResponse?.message ===
                          "No Matches Found." && !isInWatchlist ? (
                          <p className="h-4 w-4 text-sm bg-green-500 absolute right-0"></p>
                        ) : (
                          <p className="h-4 w-4 text-sm bg-red-500 absolute right-0"></p>
                        ))}
                  ...
                  {isInWatchlist && (
                    ...
                    >
                      <PiShieldWarningFill size={22} fill="orange" />
                    </button>
                  )}
                </div>
              </div>
                ...
                <div className="border border-gray-200 flex gap-2 py-2 flex-wrap justify-center w-full rounded-md min-h-[26.5rem]">
                  ...
                  >
                    <div className="w-32 h-32 border rounded-md relative">
                      <p className="h-5 w-5 flex items-center justify-center text-sm bg-white absolute font-semibold">
                        LL
                      </p>
                      {fingerprintVerificationResult2 &&
                        LeftFingerResponse?.CapturedFingers?.[0]
                          ?.FingerBitmapStr &&
                    ...
                    <div className="w-32 h-32 border rounded-md relative">
                      ...
                      {fingerprintVerificationResult3 &&
                        LeftFingerResponse?.CapturedFingers?.[1]
                          ?.FingerBitmapStr &&
                    ...
                    <div className="w-32 h-32 border rounded-md relative">
                      ...
                      {fingerprintVerificationResult4 &&
                        LeftFingerResponse?.CapturedFingers?.[2]
                          ?.FingerBitmapStr &&
                    ...
                    <div className="w-32 h-32 border rounded-md relative">
                      ...
                      {fingerprintVerificationResult5 &&
                        LeftFingerResponse?.CapturedFingers?.[3]
                          ?.FingerBitmapStr &&
                      ...
                      <button
                        onClick={() =>
                          fingerScannerCaptureLeftMutation.mutate()
                        }
                        disabled={
                          !fingerScanner || fingerScannerInitMutation.isError
                        }
                  ...
                  >
                    <div className="w-32 h-32 border rounded-md relative">
                      ...
                      {fingerprintVerificationResult7 &&
                        RightFingerResponse?.CapturedFingers?.[0]
                          ?.FingerBitmapStr &&
                    ...
                    <div className="w-32 h-32 border rounded-md relative">
                      ...
                      {fingerprintVerificationResult8 &&
                        RightFingerResponse?.CapturedFingers?.[1]
                          ?.FingerBitmapStr &&
                    ...
                    <div className="w-32 h-32 border rounded-md relative">
                      ...
                      {fingerprintVerificationResult9 &&
                        RightFingerResponse?.CapturedFingers?.[2]
                          ?.FingerBitmapStr &&
                    ...
                    <div className="w-32 h-32 border rounded-md relative">
                      ...
                      {fingerprintVerificationResult10 &&
                        RightFingerResponse?.CapturedFingers?.[3]
                          ?.FingerBitmapStr &&
                      ...
                      <button
                        onClick={() =>
                          fingerScannerCaptureRightMutation.mutate()
                        }
                        disabled={
                          !fingerScanner || fingerScannerInitMutation.isError
                        }
                    ...
                    >
                      <div className="w-32 h-32 border rounded-md relative">
                        ...
                        {fingerprintVerificationResult1 &&
                          ThumbFingerResponse?.CapturedFingers?.[0]
                            ?.FingerBitmapStr &&
                      ...
                      <div className="w-32 h-32 border rounded-md relative">
                        ...
                        {fingerprintVerificationResult6 &&
                          ThumbFingerResponse?.CapturedFingers?.[1]
                            ?.FingerBitmapStr &&
                        ...
                        <button
                          onClick={() =>
                            fingerScannerCaptureThumbMutation.mutate()
                          }
                          disabled={
                            !fingerScanner || fingerScannerInitMutation.isError
                          }
