...
const MultipleBirthSiblings = ({
  ...
  const handleEditMultipleBirthSibling = (
    ...
    setTableInfo((prev) => {
      ...
      const chosenSibling = persons?.find(
        (person) =>
          person?.id === updatedData?.sibling_person_id ||
          updatedData?.sibling_person_id_display
      );
      updatedTableInfo[index] = {
        sibling_group:
          chosenSibling?.multiple_birth_siblings?.[0]?.multiple_birth_class ||
          "",
  ...
  const handleDeleteClick = (index: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this sibling?",
  ...
  const IdentifierDataSource = tableInfo?.map((info, index) => {
    const normalizeSiblingGroup =
      info?.sibling_group === "" ? "Single" : info.sibling_group;
    console.log(info);
    return {
      key: index,
