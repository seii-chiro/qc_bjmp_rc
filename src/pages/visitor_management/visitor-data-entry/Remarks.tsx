...
const Remarks = ({
  ...
  useEffect(() => {
    ...
    if (
      remarksTableInfo.length === 0 &&
      visitorForm?.remarks_data?.length > 0
    ) {
      // Create display data with timestamps and names
      const formattedRemarks: RemarksFormType[] = visitorForm.remarks_data.map(
        (item: any, index: number) => {
          // If remarksTableInfo already has this entry with timestamp, use that
          if (
            index < remarksTableInfo.length &&
            remarksTableInfo[index].timestamp
          ) {
  ...
  const remarksDataSource =
    remarksTableInfo?.map((remarks, index) => {
      return {
        key: index,
