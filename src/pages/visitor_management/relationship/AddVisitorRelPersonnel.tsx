...
const AddVisitorRelPersonnel: React.FC<AddVisitorRelPersonnelPayload> = ({
  onClose,
}) => {
  ...
  const [selectedRelationship, setselectedRelationship] =
    useState<AddVisitorRelPersonnelPayload>({
  ...
  async function AddVisitorRelPersonnel(
    pdl_status: AddVisitorRelPersonnelPayload
  ) {
    const res = await fetch(
      `${BASE_URL}/api/non-pdl-visitor/visitor-rel-personnel/`,
      {
        method: "POST",
        headers: {
  ...
  const onImpactLevelChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setselectedRelationship((prevForm) => ({
      ...prevForm,
      impact_level_id: Number(e.target.value),
    }));
  };

  const onriskLevelChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setselectedRelationship((prevForm) => ({
      ...prevForm,
      risk_level_id: Number(e.target.value),
    }));
  };

  const onthreatLevelChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setselectedRelationship((prevForm) => ({
      ...prevForm,
      threat_level_id: Number(e.target.value),
    }));
  };
