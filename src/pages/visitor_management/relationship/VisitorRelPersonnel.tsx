...
import {
  deleteVisitorRelPersonnel,
  getThreatLevel,
  updateVisitorRelPersonnel,
} from "@/lib/query";
...
const VisitorRelPersonnel = () => {
  ...
  const [visitorRelPersonnel, setVisitorRelPersonnel] =
    useState<VisitorRelPersonnelRecord | null>(null);
  ...
  const fetchVisitorRelPersonnel = async (search: string) => {
    const res = await fetch(
      `${BASE_URL}/api/non-pdl-visitor/visitor-rel-personnel/?search=${search}`,
      {
        headers: {
  ...
  const { data, isFetching } = useQuery({
    ...
    queryFn: async (): Promise<
      PaginatedResponse<VisitorRelPersonnelRecord>
    > => {
      ...
      const res = await fetch(
        `${BASE_URL}/api/non-pdl-visitor/visitor-rel-personnel/?${params.toString()}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Token ${token}`,
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: any) => {
      messageApi.error(
        error.message || "Failed to delete Visitor Relationship Personnel"
      );
    },
  });
  ...
  const { mutate: editVisitorRelPersonnel, isLoading: isUpdating } =
    useMutation({
      ...
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ["visitor-per-personnel"] });
        messageApi.success(
          "Visitor Relationship Personnel updated successfully"
        );
  ...
  const columns: ColumnsType<VisitorRelPersonnelRecord> = [
    ...
    },
    {
      ...
      sorter: (a, b) =>
        a.relationship_personnel.localeCompare(b.relationship_personnel),
  ...
  return (
    <div>
      ...
      >
        <AddVisitorRelPersonnel onClose={handleCancel} />
      </Modal>
      <Modal
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleUpdate}>
          <div className="flex flex-wrap">
            <div className="w-full md:w-1/2 p-2">
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "Please input the Visitor Relationship Personnel",
                  },
                ]}
              >
              ...
              <Form.Item
                ...
                label="Impacts"
                rules={[
                  { required: true, message: "Please input the Impacts" },
                ]}
              >
                <Input className="h-12 border w-full border-gray-300 rounded-lg px-2" />
              </Form.Item>
              <Form.Item
                ...
                label="Threats"
                rules={[
                  { required: true, message: "Please input the Threats" },
                ]}
              >
                <Input className="h-12 border w-full border-gray-300 rounded-lg px-2" />
              </Form.Item>
              <Form.Item
                ...
                label="Mitigation"
                rules={[
                  { required: true, message: "Please input the Mitigation" },
                ]}
              >
                <Input className="h-12 border w-full border-gray-300 rounded-lg px-2" />
              </Form.Item>
            ...
            <div className="w-full md:w-1/2 p-2">
              <Form.Item label="Impact Level" name="impact_level_id">
                <Select
              ...
              <Form.Item label="Risk Level" name="risk_level_id">
                <Select
                  className="h-[3rem] w-full"
                  showSearch
              ...
              <Form.Item label="Threat Level" name="threat_level_id">
                <Select
                  className="h-[3rem] w-full"
                  showSearch
