    ...
    const loadTableData = async () => {
      ...
      const data = await Promise.all(
        visitorForm.pdl_data.map(async (pdl, index) => {
          let specificpdl = pdls?.find(
            (thispdl) => thispdl?.id === (pdl.pdl_id ?? pdl?.pdl?.id)
          );
          if (!specificpdl && (pdl.pdl_id ?? pdl?.pdl?.id)) {
            try {
              specificpdl = await fetchPDLById(
                token ?? "",
                pdl.pdl_id ?? pdl?.pdl?.id
              );
            } catch (e) {
              specificpdl = null;
            }
          }
          const specificrelationship = visitorToPdlRelationship?.find(
            (thisrelationship) =>
          ...
          return {
            ...
            annex:
              specificpdl?.cell?.cell_floor
                ?.replace(/\s*\([^)]*\)/, "")
                .trim() || pdl?.annex?.replace(/\s*\([^)]*\)/, "").trim(),
            visitationStatus:
              specificpdl?.visitation_status || pdl?.visitationStatus,
            birthClassClassification:
              specificpdl?.person?.multiple_birth_siblings?.[0]
                ?.multiple_birth_class || pdl?.birthClassClassification,
            action: (
