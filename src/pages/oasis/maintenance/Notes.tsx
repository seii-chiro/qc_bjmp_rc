import { deleteOASISNote, getOASISNotes } from "@/lib/oasis-query"
...
export type NoteDataSourceRecord = {
  id: number;
  no: number;
  note_text: string;
  description: string;
...
const Notes = () => {
  ...
  const [recordToEdit, setRecordToEdit] = useState<NoteDataSourceRecord | null>(null)
  const [searchText, setSearchText] = useState("")
  const [pdfDataUrl, setPdfDataUrl] = useState<string>('');

  const { data: notes, isLoading: notesLoading } = useQuery({
    queryKey: ['OASIS', 'notes'],
    queryFn: () => getOASISNotes(token ?? "")
  })

  const deleteNoteMutation = useMutation({
    mutationFn: (id: number) => deleteOASISNote(token ?? "", id),
    onSuccess: () => {
  ...
  const dataSource = notes?.results.map((item, index) => {
    return ({
      id: item?.id,
      no: index + 1,
  ...
  const filteredDataSource = dataSource?.filter(item => {
    ...
    return (
      item?.note_text?.toLowerCase().includes(searchLower) ||
  ...
  const handleEditOpenModal = (record: NoteDataSourceRecord) => {
    setIsFormModalOpen(true)
    setRecordToEdit(record)
  }

  const handleDelete = (record: NoteDataSourceRecord) => {
    Modal.confirm({
      centered: true,
      title: `Delete note "${record?.note_text}"?`,
      content: 'This action cannot be undone.',
      okText: 'Yes, delete it',
      okType: 'danger',
      cancelText: 'Cancel',
      onOk: async () => {
        deleteNoteMutation.mutate(record?.id)
      }
    })
  }
  ...
  const columns: ColumnsType<NoteDataSourceRecord> = [
    {
      title: 'No.',
      dataIndex: 'no',
      key: 'no',
    },
    {
      ...
      dataIndex: 'note_text',
      key: 'note_text',
      sorter: (a, b) => (a.note_text || '').toLowerCase().localeCompare((b.note_text || '').toLowerCase()),
    },
    {
      title: 'Description',
    ...
    },
    {
      ...
      title: 'Actions',
      key: 'actions',
      render: (_: unknown, record: NoteDataSourceRecord) => (
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "OASIS_Notes");
    XLSX.writeFile(wb, "OASIS_Notes.xlsx");
  };
  ...
  const items: MenuProps['items'] = [
    ...
    },
    {
      key: '2',
      label: (
        <CSVLink data={dataSource || []} filename="OASIS_Notes.csv">
          Export CSV
        </CSVLink>
      ),
    },
  ...
  return (
    <>
      ...
      <div className="text-3xl font-bold mb-6 text-[#1E365D]">Notes</div>
