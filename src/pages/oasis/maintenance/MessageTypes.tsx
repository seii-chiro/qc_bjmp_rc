import { deleteOASISMessageType, getOASISMessageTypes } from "@/lib/oasis-query"
...
import MessageTypeForm from "./forms/MessageTypeForm"

export type MessageTypeDataSourceRecord = {
  id: number;
  no: number;
  code: string;
  description: string;
...
const MessageTypes = () => {
  ...
  const [recordToEdit, setRecordToEdit] = useState<MessageTypeDataSourceRecord | null>(null)
  const [searchText, setSearchText] = useState("")
  const [pdfDataUrl, setPdfDataUrl] = useState<string>('');

  const { data: messageTypes, isLoading: messageTypesLoading } = useQuery({
    queryKey: ['OASIS', 'message-type'],
    queryFn: () => getOASISMessageTypes(token ?? "")
  })

  const deleteMessageTypeMutation = useMutation({
    mutationFn: (id: number) => deleteOASISMessageType(token ?? "", id),
    onSuccess: () => {
      message.success("Message Type deleted")
      queryClient.invalidateQueries({ queryKey: ['OASIS', 'message-type'] })
    },
    onError: () => {
      message.error("Failed to delete message type.")
    }
  })
  ...
  const handleDelete = (record: MessageTypeDataSourceRecord) => {
    Modal.confirm({
      centered: true,
      title: `Delete message type "${record?.code}"?`,
      content: 'This action cannot be undone.',
      okText: 'Yes, delete it',
      okType: 'danger',
      cancelText: 'Cancel',
      onOk: async () => {
        deleteMessageTypeMutation.mutate(record?.id)
      }
    })
  }
  ...
  const columns: ColumnsType<MessageTypeDataSourceRecord> = [
    {
      title: 'No.',
      dataIndex: 'no',
      key: 'no',
    ...
    },
    {
      ...
      title: 'Actions',
      key: 'actions',
      render: (_: unknown, record: MessageTypeDataSourceRecord) => (
  ...
  const handleGeneratePDF = () => {
    ...
    const title = "OASIS Message Type";
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "OASIS_Type");
    XLSX.writeFile(wb, "OASIS_Type.xlsx");
  };
  ...
  const items: MenuProps['items'] = [
    ...
    },
    {
      key: '2',
      label: (
        <CSVLink data={dataSource || []} filename="OASIS_Type.csv">
          Export CSV
        </CSVLink>
      ),
    },
  ...
  return (
    <>
      ...
      <div className="text-3xl font-bold mb-6 text-[#1E365D]">Message Type</div>
