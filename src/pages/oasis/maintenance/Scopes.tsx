import { deleteOASISScopes, getOASISScopes } from "@/lib/oasis-query"
...
export type ScopeDataSourceRecord = {
  id: number;
  no: number;
  code: string;
  description: string;
...
const Scopes = () => {
  ...
  const [recordToEdit, setRecordToEdit] = useState<ScopeDataSourceRecord | null>(null)
  const [searchText, setSearchText] = useState("")
  const [pdfDataUrl, setPdfDataUrl] = useState<string>('');

  const { data: scopes, isLoading: scopesLoading } = useQuery({
    queryKey: ['OASIS', 'scopes'],
    queryFn: () => getOASISScopes(token ?? "")
  })

  const deleteScopesMutation = useMutation({
    mutationFn: (id: number) => deleteOASISScopes(token ?? "", id),
    onSuccess: () => {
      message.success("Scope deleted")
      queryClient.invalidateQueries({ queryKey: ['OASIS', 'scopes'] })
    },
    onError: () => {
      message.error("Failed to delete scopes")
    }
  })

  const dataSource = scopes?.results.map((item, index) => {
    return ({
      id: item?.id,
      no: index + 1,
      code: item?.code,
  ...
  const handleEditOpenModal = (record: ScopeDataSourceRecord) => {
    setIsFormModalOpen(true)
    setRecordToEdit(record)
  }

  const handleDelete = (record: ScopeDataSourceRecord) => {
    Modal.confirm({
      centered: true,
      title: `Delete scopes "${record?.code}"?`,
      content: 'This action cannot be undone.',
      okText: 'Yes, delete it',
      okType: 'danger',
      cancelText: 'Cancel',
      onOk: async () => {
        deleteScopesMutation.mutate(record?.id)
      }
    })
  }
  ...
  const columns: ColumnsType<ScopeDataSourceRecord> = [
    {
      title: 'No.',
      dataIndex: 'no',
      key: 'no',
    ...
    },
    {
      ...
      title: 'Actions',
      key: 'actions',
      render: (_: unknown, record: ScopeDataSourceRecord) => (
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "OASIS_Scope");
    XLSX.writeFile(wb, "OASIS_Scope.xlsx");
  };
  ...
  const items: MenuProps['items'] = [
    ...
    },
    {
      key: '2',
      label: (
        <CSVLink data={dataSource || []} filename="OASIS_Scope.csv">
          Export CSV
        </CSVLink>
      ),
    },
  ...
  return (
    <>
      ...
      <div className="text-3xl font-bold mb-6 text-[#1E365D]">Scope</div>
