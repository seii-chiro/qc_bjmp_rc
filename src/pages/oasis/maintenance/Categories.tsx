import { deleteOASISCategories, getOASISCategories } from "@/lib/oasis-query"
...
export type CategoryDataSourceRecord = {
  id: number;
  no: number;
  code: string;
  description: string;
...
const Categories = () => {
  ...
  const [recordToEdit, setRecordToEdit] = useState<CategoryDataSourceRecord | null>(null)
  ...
  const deleteCategoryMutation = useMutation({
    mutationFn: (id: number) => deleteOASISCategories(token ?? "", id),
    onSuccess: () => {
  ...
  const dataSource = categories?.results.map((item, index) => {
    return ({
      id: item?.id,
      no: index + 1,
      code: item?.code,
  ...
  const handleEditOpenModal = (record: CategoryDataSourceRecord) => {
    setIsFormModalOpen(true)
    setRecordToEdit(record)
  }

  const handleDelete = (record: CategoryDataSourceRecord) => {
    Modal.confirm({
      centered: true,
      title: `Delete category "${record?.code}"?`,
  ...
  const columns: ColumnsType<CategoryDataSourceRecord> = [
    {
      title: 'No.',
      dataIndex: 'no',
      key: 'no',
    ...
    },
    {
      ...
      title: 'Actions',
      key: 'actions',
      render: (_: unknown, record: CategoryDataSourceRecord) => (
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "OASIS_Category");
    XLSX.writeFile(wb, "OASIS_Category.xlsx");
  };
  ...
  const items: MenuProps['items'] = [
    ...
    },
    {
      key: '2',
      label: (
        <CSVLink data={dataSource || []} filename="OASIS_Category.csv">
          Export CSV
        </CSVLink>
      ),
    },
  ...
  return (
    <>
      ...
      <div className="text-3xl font-bold mb-6 text-[#1E365D]">Categories</div>
