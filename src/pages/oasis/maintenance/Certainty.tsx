import { deleteOASISCertainty, getOASISCertainty } from "@/lib/oasis-query"
...
import CertaintyForm from "./forms/CertaintyForm"

export type CertaintyDataSourceRecord = {
    id: number;
    no: number;
    code: string;
    description: string;
...
const Certainty = () => {
    ...
    const [recordToEdit, setRecordToEdit] = useState<CertaintyDataSourceRecord | null>(null)
    const [searchText, setSearchText] = useState("")
    const [pdfDataUrl, setPdfDataUrl] = useState<string>('');

    const { data: certainties, isLoading: certaintiesLoading } = useQuery({
        queryKey: ['OASIS', 'certainty'],
        queryFn: () => getOASISCertainty(token ?? "")
    })

    const deleteCertaintyMutation = useMutation({
        mutationFn: (id: number) => deleteOASISCertainty(token ?? "", id),
        onSuccess: () => {
            message.success("Certainty deleted")
            queryClient.invalidateQueries({ queryKey: ['OASIS', 'certainty'] })
        },
        onError: () => {
            message.error("Failed to delete certainty")
        }
    })

    const dataSource = certainties?.results.map((item, index) => {
        return ({
            id: item?.id,
            no: index + 1,
            code: item?.code,
    ...
    const handleDelete = (record: CertaintyDataSourceRecord) => {
        Modal.confirm({
            centered: true,
            title: `Delete certainty "${record?.code}"?`,
            content: 'This action cannot be undone.',
            okText: 'Yes, delete it',
            okType: 'danger',
            cancelText: 'Cancel',
            onOk: async () => {
                deleteCertaintyMutation.mutate(record?.id)
            }
        })
    }
    ...
    const columns: ColumnsType<CertaintyDataSourceRecord> = [
        {
            title: 'No.',
            dataIndex: 'no',
            key: 'no',
        ...
        },
        {
            ...
            title: 'Actions',
            key: 'actions',
            render: (_: unknown, record: CertaintyDataSourceRecord) => (
    ...
    const handleExportExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "OASIS_Certainty");
        XLSX.writeFile(wb, "OASIS_Certainty.xlsx");
    };
    ...
    const items: MenuProps['items'] = [
        ...
        },
        {
            key: '2',
            label: (
                <CSVLink data={dataSource || []} filename="OASIS_Certainty.csv">
                    Export CSV
                </CSVLink>
            ),
        },
    ...
    return (
        <>
            ...
            <div className="text-3xl font-bold mb-6 text-[#1E365D]">Certainty</div>
