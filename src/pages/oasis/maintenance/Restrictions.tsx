import { deleteOASISRestriction, getOASISRestrictions } from "@/lib/oasis-query"
...
export type RestrictionDataSourceRecord = {
  id: number;
  no: number;
...
const Restrictions = () => {
  ...
  const [recordToEdit, setRecordToEdit] = useState<RestrictionDataSourceRecord | null>(null)
  const [searchText, setSearchText] = useState("")
  const [pdfDataUrl, setPdfDataUrl] = useState<string>('');

  const { data: restrictions, isLoading: restrictionsLoading } = useQuery({
    queryKey: ['OASIS', 'restrictions'],
    queryFn: () => getOASISRestrictions(token ?? "")
  })

  const deleteRestrictionMutation = useMutation({
    mutationFn: (id: number) => deleteOASISRestriction(token ?? "", id),
    onSuccess: () => {
      message.success("Restriction deleted")
      queryClient.invalidateQueries({ queryKey: ['OASIS', 'restrictions'] })
    },
    onError: () => {
      message.error("Failed to delete restriction.")
    }
  })
  ...
  const filteredDataSource = dataSource?.filter(item => {
    ...
    return (
      item?.restriction_text?.toLowerCase().includes(searchLower) ||
  ...
  const handleEditOpenModal = (record: RestrictionDataSourceRecord) => {
    setIsFormModalOpen(true)
    setRecordToEdit(record)
  }

  const handleDelete = (record: RestrictionDataSourceRecord) => {
    Modal.confirm({
      centered: true,
      title: `Delete restriction "${record?.restriction_text}"?`,
      content: 'This action cannot be undone.',
      okText: 'Yes, delete it',
      okType: 'danger',
      cancelText: 'Cancel',
      onOk: async () => {
        deleteRestrictionMutation.mutate(record?.id)
      }
    })
  }
  ...
  const columns: ColumnsType<RestrictionDataSourceRecord> = [
    {
      title: 'No.',
      dataIndex: 'no',
      key: 'no',
    ...
    {
      title: 'Restriction',
      dataIndex: 'restriction_text',
      key: 'restriction_text',
      sorter: (a, b) => (a.restriction_text || '').toLowerCase().localeCompare((b.restriction_text || '').toLowerCase()),
    },
    {
      title: 'Description',
    ...
    },
    {
      ...
      title: 'Actions',
      key: 'actions',
      render: (_: unknown, record: RestrictionDataSourceRecord) => (
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "OASIS_Restriction");
    XLSX.writeFile(wb, "OASIS_Restriction.xlsx");
  };
  ...
  const items: MenuProps['items'] = [
    ...
    },
    {
      key: '2',
      label: (
        <CSVLink data={dataSource || []} filename="OASIS_Restriction.csv">
          Export CSV
        </CSVLink>
      ),
    },
  ...
  return (
    <>
      ...
      <div className="text-3xl font-bold mb-6 text-[#1E365D]">Restriction</div>
