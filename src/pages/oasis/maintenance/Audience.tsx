import { deleteOASISAudience, getOASISAudience } from "@/lib/oasis-query"
...
export type AudienceDataSourceRecord = {
  id: number;
  no: number;
...
const Audience = () => {
  ...
  const [recordToEdit, setRecordToEdit] = useState<AudienceDataSourceRecord | null>(null)
  ...
  const deleteStatusMutation = useMutation({
    ...
    onError: () => {
      message.error("Failed to delete audience")
    }
  })

  const dataSource = audience?.results.map((item, index) => {
    return ({
      id: item?.id,
      no: index + 1,
  ...
  const handleDelete = (record: AudienceDataSourceRecord) => {
    Modal.confirm({
      centered: true,
      title: `Delete audience "${record?.audience_text}"?`,
  ...
  const columns: ColumnsType<AudienceDataSourceRecord> = [
    {
      title: 'No.',
      dataIndex: 'no',
      key: 'no',
    ...
    },
    {
      ...
      title: 'Actions',
      key: 'actions',
      render: (_: unknown, record: AudienceDataSourceRecord) => (
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "OASIS_Audience");
    XLSX.writeFile(wb, "OASIS_Audience.xlsx");
  };
  ...
  const items: MenuProps['items'] = [
    ...
    },
    {
      key: '2',
      label: (
        <CSVLink data={dataSource || []} filename="OASIS_Audience.csv">
          Export CSV
        </CSVLink>
      ),
    },
