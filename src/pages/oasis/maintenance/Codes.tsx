import { deleteOASISCode, getOASISCodes } from "@/lib/oasis-query"
...
const Codes = () => {
  ...
  const [recordToEdit, setRecordToEdit] = useState<CodeDataSourceRecord | null>(null)
  const [searchText, setSearchText] = useState("")
  const [pdfDataUrl, setPdfDataUrl] = useState<string>('');

  const { data: codes, isLoading: codesLoading } = useQuery({
    queryKey: ['OASIS', 'codes'],
    queryFn: () => getOASISCodes(token ?? "")
  })

  const deleteCodeMutation = useMutation({
    mutationFn: (id: number) => deleteOASISCode(token ?? "", id),
    onSuccess: () => {
  ...
  const dataSource = codes?.results.map((item, index) => {
    return ({
      id: item?.id,
      no: index + 1,
      code: item?.code,
  ...
  const handleDelete = (record: CodeDataSourceRecord) => {
    Modal.confirm({
      centered: true,
      title: `Delete code "${record?.code}"?`,
      content: 'This action cannot be undone.',
      okText: 'Yes, delete it',
      okType: 'danger',
      cancelText: 'Cancel',
      onOk: async () => {
        deleteCodeMutation.mutate(record?.id)
      }
    })
  }
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "OASIS_Codes");
    XLSX.writeFile(wb, "OASIS_Codes.xlsx");
  };
  ...
  const items: MenuProps['items'] = [
    ...
    },
    {
      key: '2',
      label: (
        <CSVLink data={dataSource || []} filename="OASIS_Codes.csv">
          Export CSV
        </CSVLink>
      ),
    },
  ...
  return (
    <>
      ...
      <div className="text-3xl font-bold mb-6 text-[#1E365D]">Codes</div>
