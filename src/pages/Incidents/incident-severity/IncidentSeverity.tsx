...
import IncidentSeverityForm from "./IncidentSeverityForm";
import { deleteIncidentSeverityLevel, getSeverityLevels } from "@/lib/incidentQueries";

export type CodeDataSourceRecord = {
  id: number;
  no: number;
  name: string;
  description: string;
...
const IncidentSeverity = () => {
  ...
  const [recordToEdit, setRecordToEdit] = useState<CodeDataSourceRecord | null>(
    null
  );
  ...
  const { data: severities, isLoading: severitiesLoading } = useQuery({
    queryKey: ["Incident", "severity"],
    queryFn: () => getSeverityLevels(token ?? ""),
  });

  const deleteSeverityMutation = useMutation({
    mutationFn: (id: number) => deleteIncidentSeverityLevel(token ?? "", id),
    onSuccess: () => {
      message.success("Incident severity deleted");
      queryClient.invalidateQueries({ queryKey: ["Incident", "severity"] });
    },
    onError: () => {
      message.error("Failed to delete severity level.");
    },
  });

  const dataSource = [...(severities?.results || [])]
    .reverse()
    .map((item, index) => ({
      id: item?.id,
      no: index + 1,
  ...
  const handleDelete = (record: CodeDataSourceRecord) => {
    Modal.confirm({
      centered: true,
      title: `Delete incident severity level "${record?.name}"?`,
      content: "This action cannot be undone.",
      okText: "Yes, delete it",
      okType: "danger",
      cancelText: "Cancel",
      onOk: async () => {
        deleteSeverityMutation.mutate(record?.id);
      },
    });
  };
  ...
  const columns: ColumnsType<CodeDataSourceRecord> = [
    {
      title: "No.",
      dataIndex: "no",
      key: "no",
    ...
    },
    {
      ...
      title: "Actions",
      key: "actions",
      render: (_: unknown, record: CodeDataSourceRecord) => (
  ...
  const handleGeneratePDF = () => {
    ...
    const title = "Incident Severity Level";
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Incident_Severity_Level");
    XLSX.writeFile(wb, "Incident_Severity_Level.xlsx");
  };
  ...
  const items: MenuProps["items"] = [
    ...
    },
    {
      key: "2",
      label: (
        <CSVLink data={dataSource || []} filename="Incident_Severity_Level.csv">
          Export CSV
        </CSVLink>
      ),
    },
  ...
  return (
    <>
      ...
      <div className="text-3xl font-bold mb-6 text-[#1E365D]">Incident Severity Levels</div>
