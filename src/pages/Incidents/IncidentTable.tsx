...
const IncidentTable = () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) =>
      fetch(`${BASE_URL}/api/incidents/incidents/${id}/`, {
        method: "DELETE",
        headers: {
    ...
    },
    {
      ...
      render: (user_assigned_to: string | null, record: any) => {
        const assignedUserId =
          users?.results?.find((user) => user.email === user_assigned_to)?.id ??
          null;
        return (
          <Select
      ...
      render: (_: any, record: any) => (
        <div style={{ display: "flex", gap: 10 }}>
          <button
            ...
            onClick={() =>
              navigate("/jvms/incidents/report", { state: record })
            }
          >
            <AiOutlineEdit />
          </button>
  ...
  const items: MenuProps["items"] = [
    ...
    },
    {
      ...
      label: (
        <CSVLink
          data={incidents?.results || []}
          filename="Watchlist_Risk_Levels.csv"
        >
          Export CSV
        </CSVLink>
      ),
