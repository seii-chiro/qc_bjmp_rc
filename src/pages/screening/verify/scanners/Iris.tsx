...
const Iris = ({ devices, deviceLoading, selectedArea }: Props) => {
  ...
  const irisScannerTimeout =
    useSystemSettingsStore((state) => state?.irisScannerTimeout) || 60;

  const [selectedDeviceId, setSelectedDeviceId] = useState<number | null>(null);
  // const [isFetching, setIsFetching] = useState(false);
  const [error, setError] = useState<Error | null>(null);

  const [irisCapturePayload, setIrisCapturePayload] =
    useState<IrisCapturePayload>({ TimeOut: irisScannerTimeout, IrisSide: 0 });
  const [irsCaptureResponse, setIrisCaptureResponse] =
    useState<IrisCaptureResponse | null>(null);
    ...
    onSuccess: (data) => {
      ...
      if (data?.ErrorDescription === "Success") {
        ...
        if (data?.ImgDataLeft || data?.ImgDataRight) {
          ...
          if (data?.ImgDataLeft) {
            verifyIrisMutation.mutate({
              template: data?.ImgDataLeft ?? "",
              type: "iris",
            });
          } else if (data?.ImgDataRight) {
            verifyIrisMutation.mutate({
              template: data?.ImgDataRight ?? "",
              type: "iris",
            });
          }
        }
          ...
          <div className="w-[90%] border border-black flex items-center justify-center">
            <div className="w-full flex flex-col items-center justify-center gap-5 p-5">
              ...
              <div className="w-full flex gap-5 justify-between">
                <div
                  className={`flex-1 border border-black ${
                    irisCapturePayload?.IrisSide === 1 ? "opacity-20" : ""
                  }`}
                >
                ...
                <div
                  className={`flex-1 border border-black ${
                    irisCapturePayload?.IrisSide === 2 ? "opacity-20" : ""
                  }`}
                >
                  <p className="w-full bg-[#1E365D] text-white text-center">
                    Right Iris
                  ...
                  <div className="h-52">
                    <img
                      src={`data:image/bmp;base64,${irsCaptureResponse?.ImgDataLeft}`}
                      className="w-full h-full object-contain"
                    />
                  </div>
                </div>
              ...
              <div className="flex">
                <Button
                  disabled={
                    irisScannerInitMutation.isError || !irisScannerReady
                  }
                  onClick={() => irisScannerCaptureMutation.mutate()}
                  loading={irisScannerInitMutation.isPending}
                  className="bg-[#1976D2] text-white px-10 py-2 rounded-md w-52 h-10"
                >
                  {!irisScannerInitMutation.isPending || irisScannerReady ? "Capture Iris" : "Device not initialized."}
                </Button>
