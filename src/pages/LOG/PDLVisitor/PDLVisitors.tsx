...
const PDLVisitors = () => {
  ...
  const dataSource = (visitorData?.results || []).map((entry, index) => {
    ...
    return {
      ...
      visitor: visitorPerson
        ? `${visitorPerson?.first_name ?? ""} ${
            visitorPerson?.last_name ?? ""
          }`.trim()
        : "N/A",
      ...
      pdl_name: pdlPerson
        ...
        : "N/A",
      relationshipToPDL:
        entry?.visitor?.pdls?.[0]?.relationship_to_pdl || "No PDL relationship",
      level: pdl?.cell?.floor?.split("(")[1]?.replace(")", "") || "N/A",
      annex: pdl?.cell?.floor?.split("(")[0]?.replace(")", "") || "N/A",
      dorm: pdl?.cell?.cell_name || "N/A",
    };
  });
  ...
  const columns: ColumnsType<any> = [
    ...
    },
    {
      ...
      render: (text) => (text ? new Date(text).toLocaleString() : "..."),
      sorter: (a, b) =>
        new Date(b.timestampOut || 0) - new Date(a.timestampOut || 0),
    },
    {
      title: "Status",
      dataIndex: "status",
