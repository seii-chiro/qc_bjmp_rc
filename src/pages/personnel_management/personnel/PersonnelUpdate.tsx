...
const patchPersonnel = async (
  pdl: Partial<PersonnelForm>,
  token: string,
  id: string
) => {
...
const PersonnelUpdate = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `${BASE_URL}/api/codes/personnel/${personnel.id}/`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const [debouncedPersonSearch, setDebouncedPersonSearch] =
    useState(personSearch);
  ...
  const { data: personsPaginated, isLoading: personsLoading } = useQuery({
    ...
    queryFn: () =>
      getPersonSearch(token ?? "", 10, debouncedPersonSearch, personPage),
  ...
  const patchPersonMutation = useMutation({
    ...
    mutationFn: () =>
      patchPerson(personForm, token ?? "", personnelData?.person?.id),
    onSuccess: async () => {
  ...
  const patchPersonnelMutation = useMutation({
    ...
    mutationFn: () =>
      patchPersonnel(personnelForm, token ?? "", personnelData?.id),
  ...
  const persons = useMemo(
    () => personsPaginated?.results || [],
    [personsPaginated]
  );
  const personsCount = useMemo(
    () => personsPaginated?.count || 0,
    [personsPaginated]
  );
    ...
    setPersonnelForm({
      ...
      remarks_data:
        personnelData?.remarks?.map((remark: any) => ({
          ...
          created_by: `${
            users?.find((user) => user?.id === remark?.personnel)?.first_name ??
            ""
          } ${
            users?.find((user) => user?.id === remark?.personnel)?.last_name ??
            ""
          }`,
      ...
      person_relationship_data:
        personnelData?.person_relationships?.map(
          (relationship: {
            ...
            address: `
                                   ${
                                     relationship?.person?.addresses?.[0]
                                       ?.region ?? ""
                                   }
                                   ${
                                     relationship?.person?.addresses?.[0]
      ...
      verified_by_id:
        users?.find((user) => user?.email === personnelData?.verified_by)?.id ??
        currentUser?.id ??
        null,
      verified_at: personnelData?.verified_at ?? "",
      approved_by_id:
        users?.find((user) => user?.email === personnelData?.approved_by)?.id ??
        null,
  ...
  return (
    <div className="bg-white rounded-md shadow border border-gray-200 py-5 px-7 w-full mb-5">
        ...
        <div className="mt-5 text-gray-700">
          ...
          <div className="flex flex-col w-full gap-2">
            <div className="flex flex-col md:flex-row gap-2">
              ...
              <div className="flex flex-col mt-2 flex-1">
                <div className="flex gap-1 font-semibold">
                  Personnel Type<p className="text-red-600">*</p>
                </div>
                <Select
            ...
            <div className="flex flex-col md:flex-row gap-2">
              ...
              <div className="flex flex-col mt-2 flex-[2]">
                <div className="flex gap-1 font-semibold">
                  Personnel Status<span className="text-red-600">*</span>
                </div>
                <Select
      ...
      <EducAttainment personForm={personForm} setPersonForm={setPersonForm} />
            ...
            <div className="flex flex-col md:flex-row w-full gap-5">
              <div className="flex flex-col mt-2 w-full">
                <div className="flex gap-1">
                  Registration Status<p className="text-red-600">*</p>
                </div>
                <Select
