...
const AffiliationType = () => {
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this affiliation type?",
  ...
  const dataSource =
    data?.results?.map((affiliation, index) => ({
      key: index + 1,
      id: affiliation?.id,
  ...
  const handleExportPDF = () => {
    ...
    const addHeader = () => {
      ...
      doc.setFontSize(16);
      doc.text("Affiliation Type Report", 10, 15);
    ...
    const tableData = (isSearching ? filteredData || [] : dataSource || []).map(
      (item, idx) => [idx + 1, item.affiliation_type, item.description]
    );
    ...
    for (let i = 0; i < tableData.length; i += maxRowsPerPage) {
      ...
      autoTable(doc, {
        head: [["No.", "Affiliation Type", "Description"]],
        body: pageData,
        startY: startY,
  ...
  return (
    <>
      {contextHolder}
      <h1 className="text-3xl font-bold text-[#1E365D]">Affiliation Type</h1>
