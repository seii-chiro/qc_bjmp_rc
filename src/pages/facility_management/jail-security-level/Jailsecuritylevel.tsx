...
const JailSecurityLevel = () => {
  ...
  const [securityLevel, setSecurityLevel] =
    useState<JailSecurityLevelReport | null>(null);
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this security level?",
  ...
  const dataSource =
    data?.results?.map((jailsecurity, index) => ({
      key: index + 1,
      id: jailsecurity?.id,
  ...
  const handleExportPDF = () => {
    ...
    const tableData = (isSearching ? filteredData || [] : dataSource || []).map(
      (item, idx) => [idx + 1, item.category_name, item.description]
    );
