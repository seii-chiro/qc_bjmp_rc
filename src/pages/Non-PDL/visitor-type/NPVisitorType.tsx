...
const NPVisitorType = () => {
  ...
  const [nonPDLVisitorType, setNonPDLVisitorType] =
    useState<NonPDLVisitorType | null>(null);
  ...
  const fetchNonPDLVisitorType = async (search: string) => {
    const res = await fetch(
      `${BASE_URL}/api/non-pdl-visitor/non-pdl-visitor-types/?search=${search}`,
      {
        headers: {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["visitor-type", "visitor-type-table", page, limit],
    queryFn: async (): Promise<PaginatedResponse<NonPDLVisitorType>> => {
      ...
      const res = await fetch(
        `${BASE_URL}/api/non-pdl-visitor/non-pdl-visitor-types/?${params.toString()}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Token ${token}`,
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: any) => {
      messageApi.error(
        error.message || "Failed to delete Non-PDL Visitor Type"
      );
    },
  });
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this visitor type?",
  ...
  const columns: ColumnsType<NonPDLVisitorType> = [
    ...
    },
    {
      ...
      sorter: (a, b) =>
        a.non_pdl_visitor_type.localeCompare(b.non_pdl_visitor_type),
    },
    {
      title: "Description",
  ...
  return (
    <div>
      ...
      >
        <AddNPVisitorType onClose={handleCancel} />
      </Modal>
      <Modal
        ...
        <Form form={form} layout="vertical" onFinish={handleUpdate}>
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Please input the Non-PDL Visitor Type",
              },
            ]}
          >
            <Input />
