...
const RecordStatus = () => {
  ...
  const [recordStatus, setRecordStatus] = useState<recordStatusReport | null>(
    null
  );
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this record status?",
  ...
  const dataSource =
    data?.results?.map((recordstatus, index) => ({
      key: index + 1,
      id: recordstatus?.id,
  ...
  const filteredData = dataSource?.filter((recordstatus) =>
    Object.values(recordstatus).some((value) =>
