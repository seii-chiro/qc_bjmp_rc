...
const SocialMedia = () => {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: any) => {
      messageApi.error(
        error.message || "Failed to delete Social Media Platforms"
      );
    },
  });
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this social media platform?",
  ...
  const dataSource =
    data?.results?.map((socialmedia, index) => ({
      key: index + 1,
      id: socialmedia?.id,
  ...
  const handleExportPDF = () => {
    ...
    const addHeader = () => {
      ...
      doc.setFontSize(16);
      doc.text("Social Media Report", 10, 15);
    ...
    for (let i = 0; i < tableData.length; i += maxRowsPerPage) {
      ...
      autoTable(doc, {
        head: [["No.", "Social Media", "Description"]],
        body: pageData,
        startY: startY,
  ...
  return (
    <div>
      ...
      <Modal
        title="Social Media Platform Report"
        open={isPdfModalOpen}
