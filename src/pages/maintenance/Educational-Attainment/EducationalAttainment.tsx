...
const EducationalAttainment = () => {
  ...
  const [educationalAttainment, setEducationalAttainment] =
    useState<EducationalAttainmentProps | null>(null);
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: any) => {
      messageApi.error(
        error.message || "Failed to delete Educational Attainment"
      );
    },
  });
  ...
  const dataSource =
    data?.results?.map((educational_attainments, index) => ({
      key: index + 1,
