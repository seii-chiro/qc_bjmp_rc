...
const Users = () => {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["users", "users-table", page, limit],
    queryFn: async (): Promise<PaginatedResponse<UsersProps>> => {
      ...
      const res = await fetch(
        `${BASE_URL}/api/user/users/?${params.toString()}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Token ${token}`,
  ...
  const onGroupRoleChange = (values: string[]) => {
    const allGroupNames =
      groupRoleData?.results?.map((group) => group.name) || [];
    ...
    <div>
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleUpdate}>
          ...
          <Form.Item name="password" label="Password" id="password">
            <Input />
          </Form.Item>
          <Form.Item name="first_name" label="First Name" id="first_name">
            <Input />
          </Form.Item>
          <Form.Item name="last_name" label="Last Name" id="last_name">
            <Input />
          </Form.Item>
