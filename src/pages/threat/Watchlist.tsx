...
const Watchlist = () => {
    ...
    try {
      ...
      const formattedData = data.results.map((watchlist) => ({
        ...
        updated_at: watchlist?.updated_at
          ? moment(watchlist.updated_at).format("YYYY-MM-DD HH:mm:ss A")
          : "",
      }));
  ...
  const deletePersonAndBiometric = async (
    token: string,
    id: number,
    personId: number
  ) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: ({ id, personId }: DeleteArgs) =>
      deletePersonAndBiometric(token ?? "", id, personId),
    onSuccess: () => {
    ...
    onError: (error: any) => {
      message.error(
        error.message || "Failed to delete Watchlist Person and Biometric"
      );
    },
  });
    ...
    Modal.confirm({
      ...
      onOk() {
        deleteMutation.mutate({
          id: record?.id,
          personId: Number(record?.person_id_display),
        });
      },
    });
  };
  ...
  const dataSource =
    WatchlistData?.results?.map((watchlist) => ({
      id: watchlist?.id,
  ...
  const columns: ColumnsType<WatchlistPerson> = [
    ...
    },
    {
      ...
      sorter: (a, b) =>
        (a.white_listed_type ?? "").localeCompare(b.white_listed_type ?? ""),
      filters: [
        ...Array.from(
          new Set(allWatchlist.map((item) => item.white_listed_type ?? ""))
        )
          .filter((type) => type)
    ...
    // Repeat for risk_level, threat_level, updated_by, etc.
    {
      ...
      filters: [
        ...Array.from(
          new Set(
            (WatchlistData?.results ?? []).map((item) => item.risk_level ?? "")
          )
        )
          .filter((level) => level)
    ...
    },
    {
      ...
      filters: [
        ...Array.from(
          new Set(
            (WatchlistData?.results ?? []).map(
              (item) => item.threat_level ?? ""
            )
          )
        )
          .filter((level) => level)
    ...
    },
    {
      ...
      filters: [
        ...Array.from(
          new Set(
            (WatchlistData?.results ?? []).map((item) => item.updated_by ?? "")
          )
        )
          .filter((name) => name)
      ...
      render: (_: any, record: WatchlistPerson) => (
        <div className="flex gap-1.5 font-semibold transition-all ease-in-out duration-200 justify-center">
          <Button
            ...
            danger
            onClick={() => {
              navigate("/jvms/threats/watchlist_registration", {
                state: record?.id,
              });
            }}
          >
