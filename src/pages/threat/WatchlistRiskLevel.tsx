...
import { deleteWatchlistRiskLevel, getWatchlistRiskLevel } from "@/lib/watchlistQueries"
import WatchlistRiskLevelForm from "./forms/WatchlistRiskLevelForm"
...
const WatchlistRiskLevel = () => {
    ...
    const { data: riskLevels, isLoading: riskLevelsLoading } = useQuery({
        queryKey: ['watchlist', 'risk-levels'],
        queryFn: () => getWatchlistRiskLevel(token ?? "")
    })

    const deleteWatchlistRiskLevelMutation = useMutation({
        mutationFn: (id: number) => deleteWatchlistRiskLevel(token ?? "", id),
        onSuccess: () => {
            message.success("Risk level deleted")
    ...
    const dataSource = [...(riskLevels?.results || [])]
        .reverse()
        .map((item, index) => ({
            id: item?.id,
            no: index + 1,
    ...
    const filteredDataSource = dataSource?.filter(item => {
        ...
        return (
            item?.risk_severity?.toLowerCase().includes(searchLower) ||
    ...
    const handleDelete = (record: tableDataSourceRecord) => {
        Modal.confirm({
            centered: true,
            title: `Delete risk level "${record?.risk_severity}"?`,
            content: 'This action cannot be undone.',
            okText: 'Yes, delete it',
            okType: 'danger',
            cancelText: 'Cancel',
            onOk: async () => {
                deleteWatchlistRiskLevelMutation.mutate(record?.id)
            }
        })
    }
    ...
    const columns: ColumnsType<tableDataSourceRecord> = [
        ...
        },
        {
            ...
            key: 'risk_severity',
            sorter: (a, b) => (a.risk_severity || '').toLowerCase().localeCompare((b.risk_severity || '').toLowerCase()),
        },
        {
            title: 'Description',
    ...
    const items: MenuProps['items'] = [
        ...
        },
        {
            key: '2',
            label: (
                <CSVLink data={dataSource || []} filename="Watchlist_Risk_Levels.csv">
                    Export CSV
                </CSVLink>
            ),
        },
    ...
    return (
        <>
            ...
            >
                <WatchlistRiskLevelForm
                    recordToEdit={recordToEdit}
                    handleClose={handleCloseModal}
                />
            </Modal>

            <div className="text-3xl font-bold mb-6 text-[#1E365D]">Watchlist Risk Levels</div>
...
export default WatchlistRiskLevel