...
const DashboardSummary = () => {
  ...
  useEffect(() => {
    const getSummaries = async () => {
      try {
        ...
        setTotalLoginsPDL(
          Object.values(pdlSummary).reduce(
            (total, item) => total + (item.logins || 0),
            0
          )
        );
        setTotalLogoutsPDL(
          Object.values(pdlSummary).reduce(
            (total, item) => total + (item.logouts || 0),
            0
          )
        );
        setTotalVisitsPDL(
          Object.values(pdlSummary).reduce(
            (total, item) => total + (item.total_visits || 0),
            0
          )
        );

        // Process Main Gate Visit Data
        const mainGateSummary =
          mainGateData.success.quarterly_visitor_logs_summary;
        setTotalLoginsMainGate(
          Object.values(mainGateSummary).reduce(
            (total, item) => total + (item.logins || 0),
            0
          )
        );
        setTotalLogoutsMainGate(
          Object.values(mainGateSummary).reduce(
            (total, item) => total + (item.logouts || 0),
            0
          )
        );
        setTotalVisitsMainGate(
          Object.values(mainGateSummary).reduce(
            (total, item) => total + (item.total_visits || 0),
            0
          )
  ...
  const visitorOtherCount = Object.entries(
    summarydata?.success?.visitor_based_on_gender?.Active || {}
  )
    .filter(([key]) => key !== "Male" && key !== "Female")
    .reduce((total, [, value]) => total + (value ?? 0), 0);

  const personnelOtherCount = Object.entries(
    summarydata?.success?.personnel_based_on_gender?.Active || {}
  )
    .filter(([key]) => key !== "Male" && key !== "Female")
    .reduce((total, [, value]) => total + (value ?? 0), 0);
  ...
  const exportToExcel = (
    ...
    const genderData = [
      ...
      {
        "PDL Count Based on Gender": "Male",
    ...
    const summaryVisitorData = [
      ...
      {
        "Summary of Visitors Based on Gender": "Male",
        Total: summarydata?.success?.visitor_based_on_gender?.Active?.Male || 0,
      },
      {
        "Summary of Visitors Based on Gender": "Female",
        Total:
          summarydata?.success?.visitor_based_on_gender?.Active?.Female || 0,
      },
      {
        "Summary of Visitors Based on Gender": "Others",
        Total: visitorOtherCount || 0,
      },
      {
        "Summary of Visitors Based on Gender": "Total",
        Total:
    ...
    const summaryPersonnelData = [
      ...
      {
        "Summary of Personnels Based on Gender": "Male",
        Total:
          summarydata?.success?.personnel_based_on_gender?.Active?.Male || 0,
      },
      {
        "Summary of Personnels Based on Gender": "Female",
        Total:
          summarydata?.success?.personnel_based_on_gender?.Active?.Female || 0,
      },
      {
        "Summary of Personnels Based on Gender": "Others",
        Total: visitorOtherCount || 0,
      },
      {
        "Summary of Personnels Based on Gender": "Total",
    ...
    const summaryBJMPDutyData = [
      ...
      {
        "BJMP Personnel On and Off-Duty Based on Status": "On-Duty",
        Total:
          summarydata?.success.personnel_count_by_status.Active["On Duty"] || 0,
      },
      {
        "BJMP Personnel On and Off-Duty Based on Status": "Off-Duty",
        Total:
          summarydata?.success.personnel_count_by_status.Active["Off Duty"] ||
          0,
      },
      {
        ...
        Total:
          summarydata?.success.personnel_count_by_status.Active["Sick Leave"] +
            summarydata?.success.personnel_count_by_status.Active[
              "Vacation Leave"
            ] +
            summarydata?.success.personnel_count_by_status.Active[
              "Maternity Leave"
    ...
    const summaryVisitorWorksheet =
      XLSX.utils.json_to_sheet(summaryVisitorData);
    const summaryPersonnelWorksheet =
      XLSX.utils.json_to_sheet(summaryPersonnelData);
    const summaryBJMPDutyWorksheet =
      XLSX.utils.json_to_sheet(summaryBJMPDutyData);
    const summaryEntryExitWorksheet = XLSX.utils.json_to_sheet(
      SummaryofEntryExitinJailPremisesData
    );
    ...
    XLSX.utils.book_append_sheet(
      workbook,
      summaryVisitorWorksheet,
      "Summary of Visitors"
    );
    XLSX.utils.book_append_sheet(
      workbook,
      summaryPersonnelWorksheet,
      "Summary of Personnels"
    );
    XLSX.utils.book_append_sheet(
      workbook,
      summaryBJMPDutyWorksheet,
      "BJMP Personnel On and Off-Duty"
    );
    XLSX.utils.book_append_sheet(
      workbook,
      summaryEntryExitWorksheet,
      "Entry or Exit in Jail Premises"
    );
  ...
  const generatePDF = async (
    ...
    const dutyBody = [
      ...
      [
        "Total",
        (summarydata?.success.personnel_count_by_status.Active["On Duty"] ||
          0) +
          (summarydata?.success.personnel_count_by_status.Active["Off Duty"] ||
            0) +
          (summarydata?.success.personnel_count_by_status.Active[
    ...
    const docDefinition = {
      ...
      content: [
        ...
        createStyledTable(
          "Summary of Entry / Exit in Jail Premises",
          entryExitBody
        ),
      ],
    ...
    pdfMake
      ...
      .download(`DashboardSummary_${formattedDate}.pdf`);
  };
  ...
  return (
    <div className="rounded-md w-full px-5 md:px-10 lg:px-32 md:py-5">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#1E365D]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 mt-5 gap-10">
        <div className="flex flex-col gap-5">
          <div className="">
            <fieldset className="border border-gray-300 rounded-md p-4 mt-2 shadow-md">
              <legend className="px-2 font-bold text-lg text-[#1E365D]">
                Summary Count of PDLs Based on their Status
              </legend>
              <div>
                <div className="overflow-hidden rounded-lg border border-gray-200 mt-2">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-100">
                      <tr>
                        <th className="px-6 py-2 text-left text-sm font-medium text-gray-500 uppercase tracking-wider border-r">
                          Status
                        </th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      <tr>
                        ...
                        <td className="px-6 py-2 text-lg whitespace-nowrap">
                          {summarydata?.success?.total_pdl_by_status?.Released
                            ?.Active || 0}
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td className="px-6 py-2 text-lg whitespace-nowrap">
                          {summarydata?.success?.total_pdl_by_status
                            ?.Hospitalized?.Active || 0}
                        </td>
                      </tr>
                      <tr>
                        <td className="px-6 py-2 text-lg whitespace-nowrap border-r">
                        ...
                        <td className="px-6 py-2 text-lg whitespace-nowrap">
                          {summarydata?.success?.total_pdl_by_status?.Convicted
                            ?.Active || 0}
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td className="px-6 py-2 text-lg whitespace-nowrap">
                          {summarydata?.success?.total_pdl_by_status?.Committed
                            ?.Active || 0}
                        </td>
                      </tr>
                      <tr>
                        <td className="px-6 py-2 text-lg whitespace-nowrap border-r bg-gray-100 font-semibold">
                          Total
                        </td>
          ...
          <div className="mt-5 ">
            <fieldset className="border border-gray-300 rounded-md p-4 mt-2 shadow-md">
              <legend className="px-2 font-bold text-lg text-[#1E365D]">
                PDL Count Based on their Gender
              </legend>
              <div>
                <div className="overflow-hidden rounded-lg border border-gray-200 mt-2">
                  <table className="min-w-full divide-y divide-gray-200">
        ...
        <div className="flex flex-col gap-5">
          <fieldset className="border border-gray-300 rounded-md p-4 mt-2 shadow-md">
            <legend className="px-2 font-bold text-lg text-[#1E365D]">
              Summary of Visitors Based on their Gender
            </legend>
            <div>
              <div className="overflow-hidden rounded-lg border border-gray-200 mt-2">
                <table className="min-w-full divide-y divide-gray-200">
          ...
          <fieldset className="border border-gray-300 rounded-md p-4 mt-2 shadow-md">
            <legend className="px-2 font-bold text-lg text-[#1E365D]">
              Summary of Personnels Based on their Gender
            </legend>
            <div>
              <div className="overflow-hidden rounded-lg border border-gray-200 mt-2">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                      ...
                      <td className="px-6 py-2 text-lg whitespace-nowrap bg-gray-100 font-semibold">
                        {summarydata?.success?.personnel_based_on_gender?.Active
                          ?.Male +
                          summarydata?.success?.personnel_based_on_gender
                            ?.Active?.Female +
                          personnelOtherCount || 0}
                      </td>
                    </tr>
                    ...
                    <tr>
                      ...
                      <td className="px-6 py-2 text-lg whitespace-nowrap bg-gray-100 font-semibold">
                        {(summarydata?.success.personnel_count_by_status.Active[
                          ...
                          (summarydata?.success.personnel_count_by_status
                            .Active["Off Duty"] || 0) +
                          (summarydata?.success.personnel_count_by_status
                            .Active["Sick Leave"] || 0) +
                          (summarydata?.success.personnel_count_by_status
                            .Active["Vacation Leave"] || 0) +
                          (summarydata?.success.personnel_count_by_status
                            .Active["Maternity Leave"] || 0) +
                          (summarydata?.success.personnel_count_by_status
                            .Active["Paternity Leave"] || 0) +
                          (summarydata?.success.personnel_count_by_status
                            .Active["Compensatory Leave"] || 0) +
                          (summarydata?.success.personnel_count_by_status
                            .Active["Absent Without Leave"] || 0)}
                      </td>
                    </tr>
                  </tbody>
                </table>
      ...
      <div className="flex justify-end gap-5">
        <button
          className="bg-[#1E365D] p-2 rounded-md text-white"
          onClick={() =>
            exportToExcel(
              summarydata,
              visitorOtherCount,
