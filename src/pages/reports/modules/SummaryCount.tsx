...
const SummaryCount = () => {
  ...
  const minorVisitorCount =
    visitorData?.results?.filter((visitor) => visitor.visitor_type === "Minor")
      .length || 0;
  const lbtqVisitorCount =
    visitorData?.results?.filter((visitor) => visitor.visitor_type === "LGBTQ+")
      .length || 0;
  ...
  const lesbianVisitorCount =
    visitorData?.results?.filter(
      (visitor) => visitor.visitor_type === "LGBTQ + LESBIAN / BISEXUAL"
    ).length || 0;
  const gayVisitorCount =
    visitorData?.results?.filter(
      (visitor) => visitor.visitor_type === "LGBTQ + GAY / BISEXUAL"
    ).length || 0;
  ...
  const lesbianCount =
    visitorData?.results?.filter(
      (visitor) =>
        visitor.person?.gender?.gender_option === "LGBTQ + LESBIAN / BISEXUAL"
    ).length || 0;
  const gayCount =
    visitorData?.results?.filter(
      (visitor) =>
        visitor.person?.gender?.gender_option === "LGBTQ + GAY / BISEXUAL"
    ).length || 0;
  const lgbtqiaCount =
    visitorData?.results?.filter(
  ...
  const exportVisitorTypeToExcel = ({
    ...
    const genderData = [
      ...
      { Gender: "LGBTQIA+", Total: gayCount + lesbianCount },
      {
        Gender: "Total",
        Total:
          maleCount + femaleCount + transgenderCount + lesbianCount + gayCount,
      },
    ];
  ...
  const exportVisitorTypeToPDF = ({
    ...
    const genderData = [
      ...
      { Gender: "LGBTQIA+", Total: lgbtqiaVisitorCount },
      {
        Gender: "Total",
        Total:
          maleCount + femaleCount + transgenderCount + lgbtqiaVisitorCount
      },
    ];
