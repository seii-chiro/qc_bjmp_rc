...
const DeviceUsage = () => {
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this device usage?",
  ...
  const dataSource =
    data?.results?.map((device_usage, index) => ({
      key: index + 1,
      id: device_usage?.id,
  ...
  const handleExportPDF = () => {
    ...
    const organizationName =
      OrganizationData?.results?.[0]?.org_name ||
    ...
    const tableData = (isSearching ? filteredData || [] : dataSource || []).map(
      (item, idx) => [idx + 1, item.usage, item.description]
    );
