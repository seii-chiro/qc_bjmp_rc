...
const ServiceProvided = () => {
  ...
  const [serviceProvided, setServiceProvided] =
    useState<ServiceProvidedPayload | null>(null);
  ...
  const fetchServiceProvided = async (search: string) => {
    const res = await fetch(
      `${BASE_URL}/api/service-providers/provided-services/?search=${search}`,
      {
        headers: {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["service-provided", "service-provided-table", page, limit],
    queryFn: async (): Promise<PaginatedResponse<ServiceProvidedPayload>> => {
      ...
      const res = await fetch(
        `${BASE_URL}/api/service-providers/provided-services/?${params.toString()}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Token ${token}`,
  ...
  return (
    <div>
      ...
      >
        <AddServiceProvided onClose={handleCancel} />
      </Modal>
      <Modal
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleUpdate}>
          <Form.Item
            ...
            rules={[
              { required: true, message: "Please input the Service Provided" },
            ]}
          >
          ...
          <Form.Item
            ...
            rules={[
              { required: true, message: "Please input the Priority Level" },
            ]}
          >
          ...
          <Form.Item
            ...
            rules={[
              { required: true, message: "Please input the Service Frequency" },
            ]}
          >
            <Select className="h-[2.8rem]">
