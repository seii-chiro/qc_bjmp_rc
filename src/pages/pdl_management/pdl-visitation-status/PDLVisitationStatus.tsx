...
import {
  deletePDLVisitationStatus,
  getPDLVisitationStatus,
  patchPDLVisitationStatus,
} from "@/lib/personnel-queries";
...
const PDLVisitationStatus = () => {
  ...
  const [pdlVisitationStatus, setpdlVisitationStatus] =
    useState<PDLStatusPayload | null>(null);
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this visitation status?",
  ...
  const handleExportPDF = () => {
    ...
    const PreparedBy =
      `${UserData?.results?.[0]?.first_name} ${UserData?.results?.[0]?.last_name}` ||
      "";
        ...
        <Form form={form} layout="vertical" onFinish={handleUpdate}>
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Please input the PDL Visitation Status name",
              },
            ]}
          >
            <Input />
