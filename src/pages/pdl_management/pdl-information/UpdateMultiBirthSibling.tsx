...
import {
  MultiBirthSiblingForm as MultiBirthSiblingFormType,
  PersonForm,
...
const UpdateMultipleBirthSiblings = ({
  ...
  const handleEditMultipleBirthSibling = (
    index: number,
    updatedData: MultiBirthSiblingFormType
  ) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const data = await Promise.all(
        personForm?.multiple_birth_sibling_data?.map(
          async (siblingData, index) => {
            let chosenSibling = persons?.find(
              (person) =>
                person?.id ===
                (siblingData?.sibling_person_id_display ||
                  siblingData?.sibling_person_id)
            );
            const siblingPersonId =
              siblingData?.sibling_person_id_display ??
              siblingData?.sibling_person_id;

            if (
              !chosenSibling &&
              siblingPersonId != null
            ) {
              try {
                chosenSibling = await fetchPersonById(siblingPersonId);
              } catch {
                chosenSibling = undefined;
              }
            }
            // console.log("Chosen Sibling", chosenSibling)
            const siblingGroup = birthClassTypes?.find(
              (type) => type?.id === siblingData?.multiple_birth_class_id
            )?.term_for_sibling_group;
            return {
              key: index,
              siblingGroup: siblingGroup || "",
              shortName:
                chosenSibling?.shortname ||
                siblingData?.sibling_person ||
                siblingData?.full_name ||
                `${chosenSibling?.first_name} ${chosenSibling?.last_name}` ||
                "N/A",
                  ...
                  <button
                    onClick={() => {
                      Modal.confirm({
                        ...
                        okType: "danger",
                        cancelText: "Cancel",
                        onOk: () => handleDeleteMultipleBirthSibling(index),
                      });
                    }}
                    type="button"
        ...
        columns={
          editIndex
            ...
            : identifierColumn.filter(
                (col) => !["gender", "siblingGroup"].includes(col.key as string)
              )
        }
        scroll={{ x: 800 }}
      />
