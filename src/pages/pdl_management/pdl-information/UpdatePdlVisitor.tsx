...
const UpdatePdlVisitor = ({ pdlForm, setPdlForm }: Props) => {
  ...
  const [debouncedVisitorSearch, setDebouncedVisitorSearch] =
    useState(visitorSearch);

  const [visitorTableData, setVisitorTableData] = useState<any[]>([]);

  useEffect(() => {
    const handler = setTimeout(
      () => setDebouncedVisitorSearch(visitorSearch),
      400
    );
    return () => clearTimeout(handler);
  }, [visitorSearch]);

  const { data: visitorsPaginated, isLoading: visitorsLoading } = useQuery({
  ...
  const {
    data: visitorToPdlRelationship,
    isLoading: visitorToPdlRelationshipLoading,
  } = useQuery({
  ...
  const handleDelete = useCallback(
    (index: number) => {
      ...
      // Also update the table data immediately
      setVisitorTableData((prev) => {
        const newTable = [...prev];
        newTable.splice(index, 1);
        return newTable;
      });
    },
    [pdlForm.visitor, setPdlForm, setVisitorTableData]
  );
        ...
        pdlForm.visitor.map(async (entry, index) => {
          try {
            const res = await fetch(
              `${BASE_URL}/api/visitors/visitor/${entry.visitor}`,
              {
                headers: {
            ...
            return {
              ...
              birthClassClassification:
                visitor?.person?.multiple_birth_siblings?.[0]
                  ?.multiple_birth_class ?? "Single",
              action: (
