...
const UpdatePDL = () => {
  ...
  const { data: personsPaginated, isLoading: personsLoading } = useQuery({
    ...
    queryFn: () =>
      getPersonSearch(token ?? "", 10, debouncedSearch, personPage),
  ...
  useEffect(() => {
    setPersonForm({
      ...
      media_data:
        pdlData?.person.media?.map((media: any) => ({
          ...media,
    ...
    setPdlForm({
      ...
      new_status_id:
        pdlStatuses?.results?.find(
          (status) => status.status === pdlData?.new_status
        )?.id ?? null,
      ...
      visitor_ids:
        pdlData?.visitor?.map((pdlVisitor: { id: any }) => pdlVisitor.id) ?? [],
      precinct_id:
  ...
  const chosenGender =
    genders?.find((gender) => gender?.id === personForm?.gender_id)
      ?.gender_option || "";
  const paddedId = String(pdlData?.id ?? "")
    .padStart(6, "0")
    .slice(-6);
  const pdlNumberDisplay = `${pdlData?.date_of_admission?.replace(
    /-/g,
    ""
  )}-${paddedId}`;
            ...
            <div className="flex flex-col md:flex-row gap-2">
              ...
              <div className="flex flex-col mt-2 flex-1">
                ...
                <Select
                  ...
                  optionFilterProp="label"
                  className="mt-2 h-10 rounded-md outline-gray-300 !bg-gray-100"
                  options={ethnicitiesProvinces?.results?.map(
                    (ethnicity: EthnicityProvince) => ({
                      value: ethnicity?.id,
