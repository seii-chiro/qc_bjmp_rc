...
const Precinct = () => {
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this precinct?",
  ...
  const dataSource =
    data?.results?.map((precincts, index) => ({
      key: index + 1,
      id: precincts?.id,
      ...
      updated_at:
        moment(precincts?.updated_at).format("YYYY-MM-DD h:mm A") ?? "N/A",
  ...
  const handleExportPDF = () => {
    ...
    const tableData = (isSearching ? filteredData || [] : dataSource || []).map(
      (item, idx) => [idx + 1, item.precinct_id, item.precinct_name]
    );
  ...
  const filteredMunicipality = municipalityData?.results?.filter(
    (city_municipality) =>
      city_municipality.province === selectPrecinct.province_id
  );
    ...
    <div>
      ...
      >
        <Form
          form={form}
          layout="vertical"
          className="grid grid-cols-1 md:grid-cols-2 md:space-x-2"
        ...
        >
          <Form.Item
            ...
            label="Precinct ID"
            rules={[
              { required: true, message: "Please input the Precinct ID" },
            ]}
          >
          ...
          <Form.Item
            ...
            label="Precinct Name"
            rules={[
              { required: true, message: "Please input the Precinct name" },
            ]}
          >
            <Input className="h-[3rem] w-full" />
          </Form.Item>
          <Form.Item
            ...
            label="Coverage Area"
            rules={[
              { required: true, message: "Please input the Coverage Area" },
            ]}
          >
            <Input className="h-[3rem] w-full" />
          </Form.Item>
          ...
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Please input the City / Municipality",
              },
            ]}
          >
            <Select
