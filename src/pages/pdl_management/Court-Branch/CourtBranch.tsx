...
const CourtBranch = () => {
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this court branch?",
  ...
  const dataSource =
    data?.results?.map((court_branch, index) => ({
      key: index + 1,
      ...
      updated_at:
        moment(court_branch?.updated_at).format("YYYY-MM-DD h:mm A") ?? "N/A",
    })) || [];
  ...
  const handleExportPDF = () => {
    ...
    const tableData = (isSearching ? filteredData || [] : dataSource || []).map(
      (item, idx) => [idx + 1, item.court, item.branch, item.judge]
    );
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleUpdate}>
          ...
          <Form.Item
            ...
            label="Court Branch"
            rules={[
              { required: true, message: "Please input the Court Branch" },
            ]}
          >
            <Input />
          </Form.Item>
