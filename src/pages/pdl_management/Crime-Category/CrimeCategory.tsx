...
const CrimeCategory = () => {
  ...
  const [selectCrimeCategory, setSelectedCrimeCategory] =
    useState<CrimeForm | null>(null);
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this crime category?",
  ...
  const columns: ColumnsType<CrimeForm> = [
    ...
    },
    {
      ...
      sorter: (a, b) =>
        a.crime_category_name.localeCompare(b.crime_category_name),
    },
    {
      title: "Description",
  ...
  const handleExportPDF = () => {
    ...
    const tableData = (isSearching ? filteredData || [] : dataSource || []).map(
      (item, idx) => [idx + 1, item.crime_category_name, item.description]
    );
        ...
        <Form form={form} layout="vertical" onFinish={handleUpdate}>
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Please input the Crime Category name",
              },
            ]}
          >
            <Input />
