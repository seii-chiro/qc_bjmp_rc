...
import { getPDLStatuses } from "@/lib/additionalQueries";
import { BASE_URL } from "@/lib/urls";
import PDLStatusForm from "./PDLStatusForm";

export type PDLStatusDataSourceRecord = {
  id: number;
  no: number;
  status: string;
  description: string;
...
async function deletePDLStatus(token: string, id: number) {
  const res = await fetch(
    `${BASE_URL}/api/pdls/pdl-status/${id}/`,
    {
      method: "DELETE",
      headers: {
  ...
  if (!res.ok) {
    throw new Error("Failed to delete PDL Status.");
  }
...
const PDLStatus = () => {
  ...
  const [recordToEdit, setRecordToEdit] =
    useState<PDLStatusDataSourceRecord | null>(null);
  const [searchText, setSearchText] = useState("");
  const [pdfDataUrl, setPdfDataUrl] = useState<string>("");

  const { data: pdlStatuses, isLoading: pdlStatusesLoading } =
    useQuery({
      queryKey: ["PDL", "Statuses"],
      queryFn: () => getPDLStatuses(token ?? ""),
    });

  const deletePDLStatusMutation = useMutation({
    mutationFn: (id: number) => deletePDLStatus(token ?? "", id),
    onSuccess: () => {
      message.success("PDL status deleted");
      queryClient.invalidateQueries({ queryKey: ["PDL", "Statuses"] });
    },
    onError: () => {
      message.error("Failed to delete PDL Statuses");
    },
  });

  const dataSource = [...(pdlStatuses?.results || [])]
    .reverse()
    .map((item, index) => ({
      id: item?.id,
      no: index + 1,
  ...
  const handleEditOpenModal = (record: PDLStatusDataSourceRecord) => {
    setIsFormModalOpen(true);
    setRecordToEdit(record);
  };

  const handleDelete = (record: PDLStatusDataSourceRecord) => {
    Modal.confirm({
      centered: true,
      title: `Delete PDL status "${record?.status}"?`,
      content: "This action cannot be undone.",
      okText: "Yes, delete it",
      okType: "danger",
      cancelText: "Cancel",
      onOk: async () => {
        deletePDLStatusMutation.mutate(record?.id);
      },
    });
  };
  ...
  const columns: ColumnsType<PDLStatusDataSourceRecord> = [
    {
      title: "No.",
      dataIndex: "no",
      key: "no",
    ...
    },
    {
      ...
      title: "Actions",
      key: "actions",
      render: (_: unknown, record: PDLStatusDataSourceRecord) => (
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "PDL_Status");
    XLSX.writeFile(wb, "PDL_Status.xlsx");
  };
  ...
  const items: MenuProps["items"] = [
    ...
    },
    {
      key: "2",
      label: (
        <CSVLink data={dataSource || []} filename="PDL_Status.csv">
          Export CSV
        </CSVLink>
      ),
    },
  ...
  return (
    <>
      ...
      <div className="text-3xl font-bold mb-6 text-[#1E365D]">PDL Status</div>
