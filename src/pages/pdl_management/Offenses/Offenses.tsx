...
const Offenses = () => {
  ...
  const dataSource =
    data?.results?.map((offense, index) => ({
      key: index + 1,
      id: offense?.id ?? "N/A",
      ...
      updated_at:
        moment(offense?.updated_at).format("YYYY-MM-DD h:mm A") ?? "N/A",
  ...
  const handleExportPDF = () => {
    ...
    const tableData = (isSearching ? filteredData || [] : dataSource || []).map(
      (item, idx) => [idx + 1, item.offense, item.description]
    );
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleUpdate}>
          <Form.Item
            ...
            label="Offense Name"
            rules={[
              { required: true, message: "Please input the Offense name" },
            ]}
          >
            <Input />
          </Form.Item>
          <Form.Item label="Crime Category" name="crime_category_id">
            <Select
          ...
          <Form.Item label="Law" name="law_id">
            <Select
              className="h-[3rem] w-full"
              showSearch
              placeholder="Law"
