...
const Relationship = () => {
  ...
  const [selectRelationship, setSelctedRelationship] =
    useState<RelationshipForm | null>(null);
  const [pdfDataUrl, setPdfDataUrl] = useState(null);
  const [isPdfModalOpen, setIsPdfModalOpen] = useState(false);

  const fetchRelationship = async (search: string) => {
    const res = await fetch(
      `${BASE_URL}/api/pdls/relationship/?search=${search}`,
      {
        headers: {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["relationship", "relationship-table", page, limit],
    queryFn: async (): Promise<PaginatedResponse<RelationshipForm>> => {
      ...
      const res = await fetch(
        `${BASE_URL}/api/pdls/relationship/?${params.toString()}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Token ${token}`,
  ...
  const dataSource =
    data?.results?.map((relationship, index) => ({
      key: index + 1,
      id: relationship?.id,
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleUpdate}>
          <Form.Item
            ...
            rules={[
              { required: true, message: "Please input the Relationship name" },
            ]}
          >
            <Input />
          </Form.Item>
