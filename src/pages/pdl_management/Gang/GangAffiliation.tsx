...
const GangAffiliation = () => {
  ...
  const [selectAffiliation, setSelctedAffiliation] =
    useState<GangAffiliationProps | null>(null);
  ...
  const showDeleteConfirm = (id: number) => {
    Modal.confirm({
      centered: true,
      title: "Are you sure you want to delete this gang affiliation?",
  ...
  const dataSource =
    data?.results?.map((gang_affiliation, index) => ({
      key: index + 1,
      ...
      updated_at:
        moment(gang_affiliation?.updated_at).format("YYYY-MM-DD h:mm A") ??
        "N/A",
        ...
        <Form form={form} layout="vertical" onFinish={handleUpdate}>
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Please input the Gang Affiliation name",
              },
            ]}
          >
            <Input />
